...
return a.geometry}function isInfinite(a){return!isFinite(a)}function RouteSegmentFailed(a){this.message=a,this.name="RouteSegmentFailed"}function init(){var a,b,c,d,e=window.innerWidth,f=window.innerHeight;hasLocalStorage()&&(storage=window.localStorage),dpr=1,void 0!==window.devicePixelRatio&&(dpr=window.devicePixelRatio),a=document.getElementById("webgl-container"),camera=new THREE.PerspectiveCamera(45,e/f,10,1600),camera.position.copy(SCMAP.settings.camera.camera),camera.setViewOffset(e,f,-($("#map_ui").width()/2),0,e,f),controls=new SCMAP.OrbitControls(camera,a),controls.target.copy(SCMAP.settings.camera.target),controls.rotateSpeed=$("#sc-map-config").data("rotateSpeed"),controls.zoomSpeed=$("#sc-map-config").data("zoomSpeed"),controls.panSpeed=$("#sc-map-config").data("panSpeed"),controls.addEventListener("change",render),controls.noRotate=SCMAP.settings.control.rotationLocked,scene=new THREE.Scene,renderer=new THREE.WebGLRenderer({antialias:!0}),SCMAP.settings.effect.Antialias||(renderer.autoClear=!1),renderer.setClearColor(0,1),renderer.setSize(window.innerWidth,window.innerHeight),a.appendChild(renderer.domElement),map=new SCMAP.Map(scene),ui=new SCMAP.UI,stats=new Stats,stats.domElement.style.position="absolute",stats.domElement.style.top="0px",stats.domElement.style.right="0px",stats.domElement.style.display="none",stats.domElement.style.zIndex="100",a.appendChild(stats.domElement),SCMAP.settings.renderer.Stats&&$("#stats").show(),window.addEventListener("resize",onWindowResize,!1),document.addEventListener("change",render),SCMAP.settings.effect.Antialias||(b=new THREE.RenderPass(scene,camera),effectFXAA=new THREE.ShaderPass(THREE.FXAAShader),effectFXAA.uniforms.resolution.value.set(1/(e*dpr),1/(f*dpr)),effectFXAA.enabled=SCMAP.settings.effect.FXAA,d=new THREE.BloomPass(.6),d.enabled=SCMAP.settings.effect.Bloom,c=new THREE.ShaderPass(THREE.CopyShader),c.renderToScreen=!0,composer=new THREE.EffectComposer(renderer),composer.setSize(e*dpr,f*dpr),composer.addPass(b),composer.addPass(effectFXAA),composer.addPass(d),composer.addPass(c)),displayState=buildDisplayModeFSM(SCMAP.settings.mode)}function smokeTest(){var a=new THREE.Geometry;for(i=0;25>i;i++){var b=new THREE.Vector3(8*Math.random(),10*Math.random()+5,8*Math.random());a.vertices.push(b)}var c=THREE.ImageUtils.loadTexture("images/smoke.png"),d=new THREE.ParticleBasicMaterial({map:c,transparent:!0,blending:THREE.AdditiveBlending,size:25,color:1118481}),e=new THREE.ParticleSystem(a,d);e.sortParticles=!0,e.position.x=10,scene.add(e)}function buildCross(){var a=new THREE.MeshBasicMaterial({wireframe:!0,color:16711680,linewidth:1}),b=new THREE.Object3D,c=new THREE.Geometry;c.vertices.push(new THREE.Vector3(-50,1,0)),c.vertices.push(new THREE.Vector3(50,1,0));var d=new THREE.Line(c,a);b.add(d),c=new THREE.Geometry;var e=new THREE.MeshBasicMaterial({wireframe:!0,color:15790080,linewidth:1});c.vertices.push(new THREE.Vector3(0,1,-50)),c.vertices.push(new THREE.Vector3(0,1,50)),d=new THREE.Line(c,e),b.add(d);var f=new THREE.MeshBasicMaterial({wireframe:!0,color:240,linewidth:1});return c=new THREE.Geometry,c.vertices.push(new THREE.Vector3(0,-50,0)),c.vertices.push(new THREE.Vector3(0,50,0)),d=new THREE.Line(c,f),b.add(d),b}function onWindowResize(){var a=window.innerWidth,b=window.innerHeight;camera.aspect=a/b,camera.setViewOffset(a,b,-($("#map_ui").width()/2),0,a,b),camera.updateProjectionMatrix(),effectFXAA&&effectFXAA.uniforms.resolution.value.set(1/(a*dpr),1/(b*dpr)),renderer.setSize(a,b),composer&&composer.reset(),$("#map_ui").data("jsp").reinitialise()}function buildDisplayModeFSM(a){var b,c,d,e;return d={y:"3d"===a?100:.5},b=new TWEEN.Tween(d).to({y:.5},1e3).easing(TWEEN.Easing.Cubic.InOut).onUpdate(function(){map.route().removeFromScene();for(var a=0;a<scene.children.length;a++){var b=scene.children[a];"function"==typeof b.userData.scaleY&&b.userData.scaleY(b,this.y)}}),c=new TWEEN.Tween(d).to({y:100},1e3).easing(TWEEN.Easing.Cubic.InOut).onUpdate(function(){map.route().removeFromScene();for(var a=0;a<scene.children.length;a++){var b=scene.children[a];"function"==typeof b.userData.scaleY&&b.userData.scaleY(b,this.y)}}),e=StateMachine.create({initial:a||"3d",events:[{name:"to2d",from:"3d",to:"2d"},{name:"to3d",from:"2d",to:"3d"}],callbacks:{onenter2d:function(){$("#3d-mode").prop("checked",!1),storage&&(storage.mode="2d")},onenter3d:function(){$("#3d-mode").prop("checked",!0),storage&&(storage.mode="3d")},onleave2d:function(){return c.onComplete(function(){e.transition(),map.route().update()}),c.start(),StateMachine.ASYNC},onleave3d:function(){return b.onComplete(function(){e.transition(),map.route().update()}),b.start(),StateMachine.ASYNC}},error:function(a,b,c,d,e,f){console.log("event "+a+" was naughty : "+f)}})}function animate(){requestAnimationFrame(animate),TWEEN.update(),void 0!==controls&&controls.update(),void 0!==editor&&editor.update(),stats.update(),render()}function render(){controls.cameraIsMoving()?(scene.updateMatrixWorld(),window.jQuery&&window.jQuery("#debug-camera-is-moving")&&window.jQuery("#debug-camera-is-moving").text("Camera is moving"),scene.traverse(function(a){a instanceof THREE.LOD&&a.update(camera)})):window.jQuery&&window.jQuery("#debug-camera-is-moving")&&window.jQuery("#debug-camera-is-moving").text("Camera is not moving"),map.animateSelector(),composer?(renderer.clear(),composer.render()):renderer.render(scene,camera)}function hasLocalStorage(){try{return"localStorage"in window&&null!==window.localStorage}catch(a){return!1}}function humanSort(a,b){var c,d,e,f=a.name.split(/(\d+)/),g=b.name.split(/(\d+)/);for(c=0;c<Math.max(f.length,g.length);c++)if(f[c]!=g[c])return d=isNaN(parseInt(f[c],10))?f[c]:parseInt(f[c],10),e=isNaN(parseInt(g[c],10))?g[c]:parseInt(g[c],10),void 0===d||void 0===e?f.length-g.length:e>d?-1:1;return 0}if("undefined"==typeof jQuery)throw new Error("starcitizen-webgl-map requires jQuery");if("undefined"==typeof THREE)throw new Error("starcitizen-webgl-map requires THREE.js");var SCMAP=SCMAP||{REVISION:"3"};self.console=self.console||{info:function(){},log:function(){},debug:function(){},warn:function(){},error:function(){}},SCMAP.data={factions:[],crime_levels:[],uee_strategic_values:[],goods:[],map:{},systems:[],systemsById:[]},SCMAP.Symbol={},SCMAP.Symbols={},SCMAP.Symbol.SIZE=24,SCMAP.Symbol.SPACING=9,SCMAP.Symbol.getTag=function(a){var b=$('<i title="'+a.description+'" class="fa fa-fw '+a.faClass+'"></i>');return b.css("color",a.color),b},SCMAP.Symbols.DANGER={code:"",scale:.9,faClass:"fa-warning",description:"Danger, hostile faction",color:"rgba(255,50,50,1.0)"},SCMAP.Symbols.WARNING={code:"",scale:.9,faClass:"fa-warning",description:"Warning, hostile environment",color:"rgba(255,117,25,1.0)"},SCMAP.Symbols.HANGAR={code:"",scale:1.15,faClass:"fa-home",description:"Hangar location",color:"rgba(255,255,255,1.0)",offset:new THREE.Vector2(-.25,2)},SCMAP.Symbols.INFO={code:"",scale:1,faClass:"fa-info-circle",description:"Information available",color:"rgba(255, 162, 255, 1.0)"},SCMAP.Symbols.TRADE={code:"",scale:.9,faClass:"fa-exchange",description:"Major trade hub",color:"rgba(255,255,0,1.0)",offset:new THREE.Vector2(0,-3)},SCMAP.Symbols.BANNED={code:"",scale:1,faClass:"fa-ban",description:"System off-limits",color:"rgba(255, 117, 25, 1.0)"},SCMAP.Symbols.AVOID={code:"",scale:1.2,faClass:"fa-times",description:"Avoid: do not route here",color:"rgba(255,50,50,1.0)"},SCMAP.Symbols.COMMENTS={code:"",scale:1,faClass:"fa-comment",description:"Your comments",color:"rgba(106, 187, 207, 1.0)",offset:new THREE.Vector2(0,-3)},SCMAP.Symbols.BOOKMARK={code:"",scale:1.05,faClass:"fa-bookmark",description:"Bookmarked",color:"rgba(102, 193, 0, 1.0)",offset:new THREE.Vector2(-1,1)},SCMAP.travelTimeAU=function(a){return SCMAP.approximateTraveltimePerAU*a},SCMAP.usersFaction=function(){return SCMAP.data.factionsByName.UEE},SCMAP.LYtoAU=63241.077,SCMAP.approximateTraveltimePerAU=2495,SCMAP.Settings=function(){this.storage=null,hasLocalStorage()&&(this.storage=window.localStorage),this.glow=this.storage&&"0"===this.storage["settings.Glow"]?!1:!0,this.labels=this.storage&&"0"===this.storage["settings.Labels"]?!1:!0,this.labelIcons=this.storage&&"0"===this.storage["settings.LabelIcons"]?!1:!0,this.camera={camera:new THREE.Vector3(0,80,100),target:new THREE.Vector3(0,10,0),orientation:{theta:0,phi:.9616764178488756,radius:122.2}},this.effect={Antialias:!0,FXAA:!1,Bloom:!1},this.control={rotationLocked:this.storage&&"1"===this.storage["control.rotationLocked"]?!0:!1},this.renderer={Stats:this.storage&&"1"===this.storage["renderer.Stats"]?!0:!1},this.route={avoidHostile:!1,avoidOffLimits:!1,avoidUnknownJumppoints:!1},this.convertAndRemoveOldSettings(),this.cameraDefaults=JSON.parse(JSON.stringify(this.camera)),this.cameraDefaults.camera=new THREE.Vector3,this.cameraDefaults.camera.copy(this.camera.camera),this.cameraDefaults.target=new THREE.Vector3,this.cameraDefaults.target.copy(this.camera.target),this.load("camera"),this.storage&&"route"in this.storage&&this.load("route"),this.load("systems"),this.systems||(this.systems={}),this.load("effect"),this.mode=this.storage&&"2d"===this.storage.mode?"2d":"3d",this.mergeAndRemoveOldSettings(),this.save("systems")},SCMAP.Settings.prototype={constructor:SCMAP.Settings,load:function(a){if(this.storage&&a in this.storage)try{this[a]=JSON.parse(this.storage[a])}catch(b){console.error("Error parsing 'localStorage."+a+"'; "+b.name+": "+b.message)}},save:function(a){this.storage&&a in this&&(this.storage[a]=JSON.stringify(this[a]))},convertAndRemoveOldSettings:function(){this.storage&&("effect.Bloom"in this.storage&&(this.effect.Bloom="1"===this.storage["effect.Bloom"]?!0:!1,delete this.storage["effect.Bloom"]),"effect.FXAA"in this.storage&&(this.effect.FXAA="1"===this.storage["effect.FXAA"]?!0:!1,delete this.storage["effect.FXAA"]),delete this.storage["camera.x"],delete this.storage["camera.y"],delete this.storage["camera.z"],delete this.storage["target.x"],delete this.storage["target.y"],delete this.storage["target.z"])},mergeAndRemoveOldSettings:function(){var a,b,c;if(this.storage)for(a in this.storage)this.storage.hasOwnProperty(a)&&(b=a.match(/^bookmarks[.](\d+)$/),b&&"1"===this.storage[a]&&(c=b[1],"object"!==this.systems[c]&&(this.systems[c]={}),this.systems[c].bookmarked=!0,delete this.storage[a]),b=a.match(/^hangarLocation[.](\d+)$/),b&&"1"===this.storage[a]&&(c=b[1],"object"!==this.systems[c]&&(this.systems[c]={}),this.systems[c].hangarLocation=!0,delete this.storage[a]),b=a.match(/^comments[.](\d+)$/),b&&""!==this.storage[a]&&(c=b[1],"object"!==this.systems[c]&&(this.systems[c]={}),this.systems[c].comments=this.storage[a],delete this.storage[a]))}},SCMAP.settings=new SCMAP.Settings,SCMAP.SelectedSystemGeometry=SCMAP.SelectedSystemGeometry||_build_selected_system_object(),SCMAP.JumpPoint=function(a){this.id=a.jumpPointId,this.name="string"==typeof a.name&&a.name.length>1?a.name:void 0,this.source=a.source instanceof SCMAP.System?a.source:void 0,this.destination=a.destination instanceof SCMAP.System?a.destination:void 0,this.drawn=!1,this.type="string"==typeof a.type?a.type:"UNDISC",this.entryAU=new THREE.Vector3,"object"==typeof a.entryAU&&Array.isArray(a.entryAU)&&(this.entryAU=this.entryAU.fromArray(a.entryAU)),this.isValid()?(void 0===this.name||""===this.name)&&(this.name="["+this.source.name+" to "+this.destination.name+"]"):console.error("Invalid route created")},SCMAP.JumpPoint.prototype={constructor:SCMAP.JumpPoint,length:function(){return this.isValid()?this.source.position.distanceTo(this.destination.position):void 0},jumpTime:function(){return this.isValid()?4*this.length():void 0},fuelConsumption:function(){return this.isValid()?0:void 0},buildSceneObject:function(){var a,b;if(!(this.drawn||(a=this.getOppositeJumppoint(),a instanceof SCMAP.JumpPoint&&a.drawn)))return b=new THREE.Geometry,b.colors.push(this.source.faction.lineColor),b.vertices.push(this.source.sceneObject.position),b.colors.push(this.destination.faction.lineColor),b.vertices.push(this.destination.sceneObject.position),this.setDrawn(),a instanceof SCMAP.JumpPoint&&a.setDrawn(),b.computeLineDistances(),new THREE.Line(b,this.getMaterial(),THREE.LinePieces)},getOppositeJumppoint:function(){for(var a=0;a<this.destination.jumpPoints.length;a++){var b=this.destination.jumpPoints[a];if(b.destination==this.source)return b}},getMaterial:function(){return this.type in SCMAP.JumpPoint.Material?SCMAP.JumpPoint.Material[this.type]:SCMAP.JumpPoint.Material.DEFAULT},isValid:function(){return this.source instanceof SCMAP.System&&this.destination instanceof SCMAP.System&&this.source!==this.destination},isUnconfirmed:function(){return"UNCONF"===this.type||"UNDISC"===this.type},setDrawn:function(){this.drawn=!0}},SCMAP.JumpPoint.Material={NORMAL:new THREE.LineBasicMaterial({color:16777215,linewidth:2,vertexColors:!0}),UNDISC:new THREE.LineDashedMaterial({color:16777215,dashSize:.75,gapSize:.75,linewidth:2,vertexColors:!0}),UNCONF:new THREE.LineDashedMaterial({color:16777215,dashSize:2,gapSize:2,linewidth:2,vertexColors:!0})},SCMAP.JumpPoint.Material.DEFAULT=SCMAP.JumpPoint.Material.UNCONF,SCMAP.Faction=function(a){this.id=void 0,this.name="Unclaimed",this.isRealFaction=!1,this.color=new THREE.Color(16777215),this.planeColor=new THREE.Color(16711680),this.lineColor=new THREE.Color(16776960),this.parentFaction=null,this.setValues(a),this._claimed={systems:{}},this._darkMaterial=void 0},SCMAP.Faction.prototype={constructor:SCMAP.Faction,claim:function(a){return!a instanceof SCMAP.System?(new Error("A faction can only claim ownership over a system"),void 0):(this._claimed.systems[a.uuid]=!0,this)},claimed:function(a){return!a instanceof SCMAP.System?(new Error("A faction can only test ownership over a system"),void 0):this._claimed.systems[a.uuid]},material:function(){return"undefined"==typeof this._darkMaterial&&(this._darkMaterial=new THREE.MeshBasicMaterial({color:this.planeColor,vertexColors:!0})),this._darkMaterial},isHostileTo:function(a){if(!(a instanceof SCMAP.Faction))throw"Can only compare to other factions";return"Vanduul"===a.name?"Vanduul"!==this.name:"Vanduul"===this.name},getValue:function(a){if(void 0!==a){var b=this[a];return b}},setValues:function(a){if(void 0!==a)for(var b in a){var c=a[b];if(void 0!==c){if(b in this){var d=this[b];d instanceof THREE.Color?(c instanceof THREE.Color?this[b]=c:(c=c.replace("0x","#"),this[b]=new THREE.Color(c)),"color"===b&&(this.planeColor=this[b].clone().offsetHSL(0,.5,0).multiplyScalar(.2),this.lineColor=this[b].clone().offsetHSL(0,.05,-.05))):this[b]=c}}else console.log('SCMAP.Faction: "'+b+'" parameter is undefined for "'+this.name+'"')}}},SCMAP.Faction.preprocessFactions=function(a){var b,c;SCMAP.data.factionsByName={};for(b in a)a.hasOwnProperty(b)&&(c=a[b],c instanceof SCMAP.Faction||(c=new SCMAP.Faction({id:b,name:c.name,color:c.color,isRealFaction:c.isRealFaction,parentFaction:null})),SCMAP.data.factions[b]=c,SCMAP.data.factionsByName[c.id]=c,SCMAP.data.factionsByName[c.name]=c)},SCMAP.Faction.getById=function(a){var b=SCMAP.data.factions[a];return b instanceof SCMAP.Faction||(b=SCMAP.data.factionsByName.Unclaimed),b},SCMAP.Faction.getByName=function(a){var b=SCMAP.data.factionsByName[a];return b instanceof SCMAP.Faction||(b=SCMAP.data.factionsByName.Unclaimed),b},SCMAP.Goods=function(a){this.id=void 0,this.name="Unknown",this.blackMarket=!1,this.setValues(a),this._trade={importing:[],exporting:[],blackMarket:[]}},SCMAP.Goods.prototype={constructor:SCMAP.Goods,getValue:function(a){if(void 0!==a){var b=this[a];return b}},setValues:function(a){if(void 0!==a)for(var b in a){var c=a[b];if(void 0!==c){if(b in this){{this[b]}this[b]=c}}else console.log('SCMAP.Goods: "'+b+'" parameter is undefined for "'+this.name+'"')}}},SCMAP.Goods.preprocessGoods=function(){var a,b;SCMAP.data.goodsByName={};for(a in SCMAP.data.goods)if(SCMAP.data.goods.hasOwnProperty(a)){if(b=SCMAP.data.goods[a],b instanceof SCMAP.Goods){SCMAP.data.goodsByName[b.name]=b;continue}b=new SCMAP.Goods({id:a,name:b.name,blackMarket:b.blackMarket}),SCMAP.data.goods[a]=b,SCMAP.data.goodsByName[b.name]=b}},SCMAP.Goods.getById=function(a){return SCMAP.data.goods[a]},SCMAP.Goods.getByName=function(a){return SCMAP.data.goodsByName[a]},SCMAP.System=function(a){this.id=void 0,this.uuid=void 0,this.name="",this.nickname="",this.position=new THREE.Vector3,this.faction=new SCMAP.Faction,this.size="medium",this.jumpPoints=[],this.poi=[],this.color=new THREE.Color(16777215),this.planets=0,this.planetaryRotation=[],this.import=[],this.export=[],this.crimeStatus="",this.blackMarket=[],this.ueeStrategicValue=void 0,this.info=[],this.scale=1,this.binary=!1,this.isOffLimits=!1,this.hasWarning=!1,this.isMajorTradeHub=!1,this.setValues(a),this._routeObjects=[],this._drawnText="",this._drawnSymbols=""},SCMAP.System.prototype={constructor:SCMAP.System,buildSceneObject:function(){var a,b,c,d,e,f;for(this.sceneObject=new THREE.Object3D,a=new THREE.Mesh(SCMAP.System.CUBE,SCMAP.System.CUBE_MATERIAL),a.visible=!1,f=Math.min(5.75,Math.max(3.5,5*this.scale)),a.scale.set(f,f,f),this.sceneObject.add(a),e=new THREE.LOD,i=0;i<SCMAP.System.LODMESH.length;i++)a=new THREE.Mesh(SCMAP.System.LODMESH[i][0],this.starMaterial()),a.scale.set(this.scale,this.scale,this.scale),a.updateMatrix(),a.matrixAutoUpdate=!1,e.addLevel(a,SCMAP.System.LODMESH[i][1]);return e.updateMatrix(),e.matrixAutoUpdate=!1,this.sceneObject.add(e),c=new THREE.Sprite(this.glowMaterial()),c.scale.set(SCMAP.System.GLOW_SCALE*this.scale,SCMAP.System.GLOW_SCALE*this.scale,1),c.userData.isGlow=!0,c.sortParticles=!0,c.visible=SCMAP.settings.glow,this.sceneObject.add(c),b=new THREE.Sprite(this.labelSprite(SCMAP.settings.labelIcons)),b.position.set(0,3.5,0),b.position.set(0,3*this.scale,0),b.scale.set(SCMAP.System.LABEL_SCALE*b.material.map.image.width,SCMAP.System.LABEL_SCALE*b.material.map.image.height,1),b.userData.isLabel=!0,b.sortParticles=!0,b.visible=SCMAP.settings.labels,this.sceneObject.add(b),d=this.position.clone(),storage&&"2d"===storage.mode&&d.setY(.005*d.y),this.sceneObject.position=d,this.sceneObject.userData.system=this,this.sceneObject.userData.scaleY=this.scaleY,this.sceneObject},updateSceneObject:function(){for(var a=0;a<this.sceneObject.children.length;a++){var b=this.sceneObject.children[a];b.userData.isLabel?(this.updateLabelSprite(b.material,SCMAP.settings.labelIcons),b.visible=SCMAP.settings.labels):b.userData.isGlow&&(b.visible=SCMAP.settings.glow)}},setLabelScale:function(a){for(var b=0;b<this.sceneObject.children.length;b++)this.sceneObject.children[b].userData.isLabel&&this.sceneObject.children[b].scale.copy(a)},starMaterial:function(){return SCMAP.System.STAR_MATERIAL_WHITE},glowMaterial:function(){var a=this.color;return a.equals(SCMAP.Color.BLACK)&&a.copy(SCMAP.Color.UNSET),new THREE.SpriteMaterial({map:SCMAP.System.GLOW_MAP,blending:THREE.AdditiveBlending,transparent:!1,useScreenCoordinates:!1,color:a})},labelSprite:function(a){var b,c,d;SCMAP.UI.fontAwesomeIsReady||(a=!1);var e=a?this.getIcons():[];return b=this.drawSystemText(this.name,e),c=new THREE.Texture(b),c.needsUpdate=!0,d=new THREE.SpriteMaterial({map:c,useScreenCoordinates:!1,blending:THREE.CustomBlending})},updateLabelSprite:function(a,b){var c,d,e=b?this.getIcons():[],f=this.iconsToKey(e);(this._drawnText!==this.name||this._drawnSymbols!==f)&&(c=this.drawSystemText(this.name,e),d=new THREE.Texture(c),d.needsUpdate=!0,a.map=d)},drawSystemText:function(a,b){var c,d,e,f,g;for(c=document.createElement("canvas"),c.width=256,c.height=256,d=c.getContext("2d"),d.font="36pt Electrolize, Calibri, sans-serif",d.textAlign="center",d.strokeStyle="rgba(0,0,0,1.0)",d.lineWidth=5,e=Math.ceil(d.measureText(a).width+1);e>c.width;)c.width*=2,c.height*=2;return f=c.width/2,g=c.height/2,d.font="36pt Electrolize, Calibri, sans-serif",d.strokeStyle="rgba(0,0,0,1.0)",d.textAlign="center",d.lineWidth=5,d.strokeText(a,f,g),d.fillStyle=this.faction.color.getStyle(),d.fillText(a,f,g),this._drawnText=a,this._drawnSymbols="",b&&b.length&&(this._drawnSymbols=this.iconsToKey(b),this._drawSymbols(d,f,g-50,b)),c},_drawSymbols:function(a,b,c,d){var e,f,g,h,i=SCMAP.Symbol.SIZE*d.length+SCMAP.Symbol.SPACING*(d.length-1);for(b-=i/2,e=0;e<d.length;e++)f=d[e],g=0,h=0,f.offset&&(g=f.offset.x,h=f.offset.y),a.font=(SCMAP.Symbol.SIZE*f.scale).toFixed(1)+"pt FontAwesome",a.strokeStyle="rgba(0,0,0,1.0)",a.textAlign="center",a.lineWidth=5,a.strokeText(f.code,b+g+SCMAP.Symbol.SIZE/2,c+h),a.fillStyle=f.color,a.fillText(f.code,b+g+SCMAP.Symbol.SIZE/2,c+h),b+=SCMAP.Symbol.SIZE+SCMAP.Symbol.SPACING},createInfoLink:function(a,b){var c=$("<a></a>");if("undefined"!=typeof this.faction&&"undefined"!=typeof this.faction&&c.css("color",this.faction.color.getStyle()),c.addClass("system-link"),c.attr("data-goto","system"),c.attr("data-system",this.id),c.attr("href","#system="+encodeURI(this.name)),c.attr("title","Show information on "+this.name),b?c.text(this.name):c.html('<i class="fa fa-crosshairs"></i>&nbsp;'+this.name),!a){var d=this.getIcons();if(d.length){for(var e=$('<span class="icons"></span>'),f=0;f<d.length;f++)e.append(SCMAP.Symbol.getTag(d[f]));c.append(e)}}return c},iconsToKey:function(a){for(var b=[],c=0;c<a.length;c++)b.push(a[c].code);return b.join(";")},getIcons:function(){var a=[];return this.faction.isHostileTo(SCMAP.usersFaction())&&a.push(SCMAP.Symbols.DANGER),this.hasWarning&&a.push(SCMAP.Symbols.WARNING),this.info.length&&a.push(SCMAP.Symbols.INFO),this.isMajorTradeHub&&a.push(SCMAP.Symbols.TRADE),this.isOffLimits&&a.push(SCMAP.Symbols.BANNED),this.hasHangar()&&a.push(SCMAP.Symbols.HANGAR),this.isBookmarked()&&a.push(SCMAP.Symbols.BOOKMARK),this.isToBeAvoided()&&a.push(SCMAP.Symbols.AVOID),this.hasComments()&&a.push(SCMAP.Symbols.COMMENTS),a},displayInfo:function(a){var b=$("#info");b.empty(),b.append(SCMAP.UI.Templates.systemInfo({system:this})),b.find(".user-system-ishangar").prop("checked",this.hasHangar()).attr("data-system",this.id),b.find(".user-system-bookmarked").prop("checked",this.isBookmarked()).attr("data-system",this.id),b.find(".user-system-avoid").prop("checked",this.isToBeAvoided()).attr("data-system",this.id),this.hasComments()?(b.find(".user-system-comments").empty().val(this.getComments()),b.find(".user-system-comments-md").html($(markdown.toHTML(this.getComments())))):(b.find(".user-system-comments").empty().val(""),b.find(".user-system-comments-md").empty()),a||($("#map_ui").tabs("option","active",2),$("#map_ui").data("jsp").scrollToPercentY(0),$("#map_ui").data("jsp").reinitialise())},scaleY:function(a,b){var c=a.userData.system.position.y*(b/100);return a.userData.system.sceneObject.translateY(c-a.userData.system.sceneObject.position.y),a.userData.system.routeNeedsUpdate(),this},moveTo:function(a){return this.system.sceneObject.position.copy(a),this.system.routeNeedsUpdate(),this},translateVector:function(a){return this.system.sceneObject.add(a),this.system.routeNeedsUpdate(),this},routeNeedsUpdate:function(){for(var a=0;a<this._routeObjects.length;a++)this._routeObjects[a].geometry.verticesNeedUpdate=!0;return this},jumpPointTo:function(a){for(var b=0;b<this.jumpPoints.length;b++)if(this.jumpPoints[b].destination===a)return this.jumpPoints[b]},isBookmarked:function(){return this.storedSettings().bookmarked===!0},setBookmarkedState:function(a){return this.storedSettings().bookmarked=a?!0:!1,this.saveSettings(),this},hasHangar:function(){return this.storedSettings().hangarLocation===!0},setHangarState:function(a){return this.storedSettings().hangarLocation=a?!0:!1,this.saveSettings(),this},isToBeAvoided:function(){return this.storedSettings().avoid===!0},setToBeAvoidedState:function(a){return this.storedSettings().avoid=a?!0:!1,this.saveSettings(),this},hasComments:function(){return"string"==typeof this.storedSettings().comments&&this.storedSettings().comments.length>0},getComments:function(){return this.storedSettings().comments},setComments:function(a){return"string"==typeof a&&a.length>1?this.storedSettings().comments=a:delete this.storedSettings().comments,this.saveSettings(),this},storedSettings:function(){return this.id in SCMAP.settings.systems||(SCMAP.settings.systems[this.id]={}),SCMAP.settings.systems[this.id]},saveSettings:function(){return SCMAP.settings.save("systems"),this},toString:function(){return this.name},getValue:function(a){if(void 0!==a){var b=this[a];return b}},_fixJumpPoints:function(a){var b,c,d,e=[];for(b=0;b<this.jumpPoints.length;b++)c=this.jumpPoints[b],c instanceof SCMAP.JumpPoint||(d=SCMAP.System.getById(c.destinationSystemId),d instanceof SCMAP.System&&(c=new SCMAP.JumpPoint({source:this,destination:d,name:c.name,type:c.type,entryAU:c.coordsAu}),a?e.push(c):system.jumpPoints[b]=c));return a&&(this.jumpPoints=e),this},setValues:function(a){var b,c,d;if(void 0!==a)for(b in a)if(d=a[b],void 0!==d){if(b in this){if(c=this[b],"size"==b){switch(d){case"dwarf":this.scale=.6;break;case"medium":this.scale=1;break;case"large":this.scale=1.25;break;case"giant":this.scale=1.6;break;case"binary":this.scale=1.6,this.binary=!0}this[b]=d}c instanceof THREE.Color?d instanceof THREE.Color?this[b]=d:(d=d.replace("0x","#"),this[b]=new THREE.Color(d)):c instanceof SCMAP.Faction?this[b]=d.claim(this):c instanceof THREE.Vector3&&d instanceof THREE.Vector3?c.copy(d):c instanceof THREE.Vector3?d instanceof THREE.Vector3?c.copy(d):d instanceof Array&&c.fromArray(d):this[b]=d}}else console.log('SCMAP.System: "'+b+'" parameter is undefined for "'+this.name+'"')}},SCMAP.System.preprocessSystems=function(a){var b,c;SCMAP.data.systems={},SCMAP.data.systemsById={},SCMAP.System.List=[];for(b in a)a.hasOwnProperty(b)&&(c=SCMAP.System.fromJSON(a[b]),SCMAP.data.systems[c.name]=c,SCMAP.data.systemsById[c.id]=c,SCMAP.data.systemsById[c.uuid]=c,SCMAP.System.List.push(c));$(SCMAP.System.List).each(function(a,b){b._fixJumpPoints(!0)})},SCMAP.System.List=[],SCMAP.System.fromJSON=function(a){return a instanceof SCMAP.System?a:new SCMAP.System({id:a.systemId,uuid:a.uuid,name:a.name,position:a.coords,scale:a.scale||1,color:a.color,faction:SCMAP.Faction.getById(a.factionId),isMajorTradeHub:a.isMajorTradeHub,hasWarning:a.hasWarning,isOffLimits:a.isOffLimits,nickname:a.nickname,size:a.size,info:a.info,crimeStatus:SCMAP.data.crimeLevels[a.crimeLevel],ueeStrategicValue:SCMAP.data.ueeStrategicValues[""+a.ueeStrategicValue],"import":a.import,"export":a.export,blackMarket:a.blackMarket,planets:[],planetaryRotation:[],jumpPoints:a.jumpPoints})},SCMAP.System.SortedList=function(){for(var a=[],b=SCMAP.System.List.length;b--;)a[b]=SCMAP.System.List[b];var c=a.sort(humanSort);return c},SCMAP.System.getByName=function(a){return SCMAP.data.systems[a]},SCMAP.System.getById=function(a){return SCMAP.data.systemsById[a]},SCMAP.Color={},SCMAP.Color.BLACK=new THREE.Color("black"),SCMAP.Color.UNSET=new THREE.Color(8429772),SCMAP.System.COLORS={RED:16736352,BLUE:6316287,WHITE:16777215,YELLOW:16777056,ORANGE:15790208,UNKNOWN:16777215},SCMAP.System.LABEL_SCALE=.06,SCMAP.System.GLOW_SCALE=6.5,SCMAP.System.CUBE=new THREE.BoxGeometry(1,1,1),SCMAP.System.LODMESH=[[new THREE.IcosahedronGeometry(1,3),20],[new THREE.IcosahedronGeometry(1,2),50],[new THREE.IcosahedronGeometry(1,1),150]],SCMAP.System.STAR_MATERIAL_WHITE=new THREE.MeshBasicMaterial({color:SCMAP.System.COLORS.WHITE,name:"STAR_MATERIAL_WHITE"}),SCMAP.System.CUBE_MATERIAL=new THREE.MeshBasicMaterial({color:16777215,transparent:!0}),SCMAP.System.CUBE_MATERIAL.opacity=.3,SCMAP.System.CUBE_MATERIAL.transparent=!0,SCMAP.System.GLOW_MAP=new THREE.ImageUtils.loadTexture($("#sc-map-config").data("glow-image")),SCMAP.Dijkstra=function(a,b,c){if("object"!=typeof a||!Array.isArray(a))return console.error("No array specified to SCMAP.Dijkstra constructor!"),void 0;for(this.start=b instanceof SCMAP.System?b:null,this.end=this.start&&c instanceof SCMAP.System?c:null,this._nodes=[],this._mapping={},i=a.length;i--;)this._nodes[i]={system:a[i],distance:Number.POSITIVE_INFINITY,previous:null},this._mapping[a[i].id]=this._nodes[i];this._result={}},SCMAP.Dijkstra.prototype={constructor:SCMAP.Dijkstra,buildGraph:function(a,b){{var c,d,e,f,g,h,i;new Date}if(!(this.start instanceof SCMAP.System))throw new Error("No source given");if(!(this.end instanceof SCMAP.System))throw new Error("No or invalid destination given");if(this._result.destination=this.end,b||!(this._result.source instanceof SCMAP.System)||this._result.source!==this.start||this._result.priority!==a){for(this.destroyGraph(),this._result.source=this.start,this._result.destination=this.end,this._result.priority=a,d=0;d<this._nodes.length;d++)this._nodes[d].distance=Number.POSITIVE_INFINITY,this._nodes[d].previous=null;for(f=this._mapping[this.start.id],f.distance=0,f.previous=null,c=SCMAP.Dijkstra.quickSort(this._nodes);c.length&&(f=c[0],f.system!==this.end)&&(c.splice(0,1),!isInfinite(f.distance));)for(d=0;d<f.system.jumpPoints.length;d++)if(g=f.system.jumpPoints[d],h=this._mapping[g.destination.id],!g.isUnconfirmed()||!SCMAP.settings.route.avoidUnknownJumppoints){if(!this.isStartOrEnd(h.system)){if(SCMAP.settings.route.avoidHostile&&!f.system.faction.isHostileTo(SCMAP.usersFaction())&&h.system.faction.isHostileTo(SCMAP.usersFaction()))continue;if(SCMAP.settings.route.avoidOffLimits&&h.system.isOffLimits)continue;if(!f.system.isToBeAvoided()&&h.system.isToBeAvoided())continue}e=f.distance+g.jumpTime(),e+=null===f.previous?SCMAP.travelTimeAU(.35):SCMAP.travelTimeAU(.7),SCMAP.settings.route.avoidHostile&&h.system.faction.isHostileTo(SCMAP.usersFaction())&&(e*=15),e<h.distance&&(h.distance=e,h.previous=f,c=SCMAP.Dijkstra.quickSort(c))}this._result.nodes=c,this._result.priority=a,i=new Date}},isStartOrEnd:function(a){return a instanceof SCMAP.System?a===this.start||a===this.end:!1},firstNode:function(){var a=this.routeArray();return a[0]},lastNode:function(){var a=this.routeArray();return a[a.length-1]},source:function(){return this.start instanceof SCMAP.System?this.start:void 0},destination:function(){return this.end instanceof SCMAP.System?this.end:void 0},rebuildGraph:function(){return this.destroyGraph(),this.start instanceof SCMAP.System?(this.buildGraph("time",!0),!0):void 0},destroyGraph:function(){this._result={}},routeArray:function(a){if(!(a instanceof SCMAP.System)){if(!(this._result.destination instanceof SCMAP.System))return console.error("No or invalid destination specified."),void 0;a=this._result.destination}if(this._result.nodes.length>0){for(var b=[],c=this._mapping[a.id],d={};null!==c;)d[c.system.name]=!0,b.push(c),c=c.previous;return b.reverse(),b}}},SCMAP.Dijkstra.quickSort=function(a){for(var b=[],c=a.length;c--;)b[c]=a[c];if(b.length<=1)return b;var d=[],e=[],f=Math.ceil(b.length/2)-1;for(f=b.splice(f,1)[0],c=0;c<b.length;c++)b[c].distance<=f.distance?d.push(b[c]):e.push(b[c]);var g=SCMAP.Dijkstra.quickSort(d),h=SCMAP.Dijkstra.quickSort(e);return g.push(f),g.concat(h)},SCMAP.Route=function(a,b){if(this.start=a instanceof SCMAP.System?a:null,this.waypoints=[],this._graphs=[],this._routeObject=void 0,this._error=void 0,b instanceof SCMAP.System)this.waypoints=[b];else if(Array.isArray(b))for(var c=0;c<b.length;c+=1)b[c]instanceof SCMAP.System&&this.waypoints.push(b[c]);this.__syncGraphs()},SCMAP.Route.prototype={constructor:SCMAP.Route,__findGraphs:function(a){for(var b=[],c={},d=0;d<this._graphs.length;d+=1){var e=[];try{e=this._graphs[d].routeArray()}catch(f){console.error("Error getting route array: "+f.message)}if(b.length&&e[0].system.id===a.id)return b.push(this._graphs[d]),b;for(var g=0;g<e.length;g+=1)e[g].system!==a||c[e[g].system.id]||(c[e[g].system.id]=!0,b.push(this._graphs[d]))}return b},splitAt:function(a){var b=this.__findGraphs(a);if(b.length>1)return console.error("Can't split at '"+a.name+"', graphs are already split"),!1;if(1!==b.length)return console.error("Couldn't find graph for waypoint '"+a.name+"'"),!1;var c=b[0],d=(c.routeArray(),c.lastNode().system);c.end=a;for(var e=0;e<this._graphs.length;e+=1)if(this._graphs[e]===c){this._graphs.splice(e+1,0,new SCMAP.Dijkstra(SCMAP.System.List,a,d));
for(var f=0;f<this.waypoints.length;f+=1)if(this.waypoints[f]===d){this.waypoints.splice(f,0,a);break}return this.__syncGraphs(),!0}console.error("Couldn't match graph to split")},toString:function(){var a=[];return this.start instanceof SCMAP.System&&a.push(this.start.toString()),$.each(this.waypoints,function(b,c){c instanceof SCMAP.System&&a.push(c)}),a.join(" > ")},removeWaypoint:function(a){var b=this.__findGraphs(a);if(2!==b.length)return console.error("Can't remove waypoint '"+a.name+"', it is not a waypoint"),!1;var c=b[0],d=b[1];c.end=d.start;for(var e=0;e<this._graphs.length;e+=1)if(this._graphs[e]===d){console.log("Matched "+d+" starting at index "+e),console.log(this._graphs),this._graphs.splice(e,1),console.log(this._graphs);for(var f=0;f<this.waypoints.length;f+=1)if(this.waypoints[f]===a){console.log("Matched "+a+" at index "+f),console.log(this.waypoints),this.waypoints.splice(f,1),console.log(this.waypoints);break}return this.__syncGraphs(),!0}},moveWaypoint:function(a,b){var c;return a===b?!1:b===this.start||this.waypoints.indexOf(b)>=0?!1:a===this.start?1!==this.waypoints.length||b!==this.waypoints[0]?(this.start=b,this.__syncGraphs(),!0):!1:(c=this.waypoints.indexOf(a),c>-1?(this.waypoints[c]=b,this.__syncGraphs(),!0):this.splitAt(a)&&(c=this.waypoints.indexOf(a),c>-1)?(this.waypoints[c]=b,this.__syncGraphs(),!0):!1)},setRoute:function(){var a,b=Array.prototype.slice.call(arguments);if(this.start=b.shift(),this.start=this.start instanceof SCMAP.System?this.start:null,this.waypoints=[],this.start){for(a=0;a<b.length;a+=1)b[a]instanceof SCMAP.System&&this.waypoints.push(b[a]);for(a=0;a<this.waypoints.length;a+=1)this.waypoints[a]=this.waypoints[a]instanceof SCMAP.System?this.waypoints[a]:null}},__syncGraphs:function(){var a=[];this._graphs=a,this._error=void 0;try{for(var b=0;b<this.waypoints.length;b+=1){var c,d=0===b?this.start:this.waypoints[b-1],e=this.waypoints[b];this._graphs[b]instanceof SCMAP.Dijkstra?(c=this._graphs[b],this._graphs[b].start=d,this._graphs[b].end=e):c=new SCMAP.Dijkstra(SCMAP.System.List,d,e),c.buildGraph("time",!0),a.push(c);var f=c.routeArray();if(f.length<=1)throw console.warn("No route from "+d.name+" to "+e.name+" possible"),new RouteSegmentFailed("No route from "+d.name+" to "+e.name+" available")}this._graphs=a,a.length>0&&console.log("Synced and built "+a.length+" graphs")}catch(g){this._error=g,g instanceof RouteSegmentFailed||console.error("Error building route: "+g.message)}},lastError:function(){return this._error},isSet:function(){var a=this.currentRoute();return a.length>1},currentRoute:function(){for(var a=[],b=0;b<this._graphs.length;b+=1)if(this.waypoints[b]instanceof SCMAP.System){this._graphs[b].rebuildGraph();for(var c=this._graphs[b].routeArray(this.waypoints[b]),d=0;d<c.length;d+=1)a.push(c[d])}return a},alphaOfSystem:function(a){if(!a instanceof SCMAP.System)return 0;var b=0,c=this.currentRoute();if(c.length)for(var d=0;d<c.length;d++)if(c[d].system===a){b=d;break}return b?b/c.length:0},indexOfCurrentRoute:function(a){if(!(!a instanceof SCMAP.System)){var b,c=this.currentRoute();if(c.length)for(var d=0;d<c.length;d++)if(c[d].system===a){b=d;break}return b}},rebuildCurrentRoute:function(){this.removeFromScene();for(var a=0;a<this._graphs.length;a++)if(this._graphs[a].rebuildGraph()){var b=this._graphs[a].destination();b&&(console.log("Have existing destination, updating route"),this.update(b))}},destroy:function(){this.start=null,this.waypoints=[],this.update()},removeFromScene:function(){this._routeObject&&scene.remove(this._routeObject),$("#routelist").empty()},update:function(a){var b,c,d,e,f=0,g=0,h=this.toString();if(this.__syncGraphs(),!(a instanceof SCMAP.System)){var i=this.waypoints.length;a=this.waypoints[i-1]}if(this.removeFromScene(),this.lastError())return $("#routelist").html('<p class="impossible">'+this.lastError().message+"</p>"),$("#map_ui").tabs("option","active",3),void 0;this._routeObject=new THREE.Object3D,this._routeObject.matrixAutoUpdate=!1;var j=this.currentRoute(),k=new THREE.Color(15658598),l=new THREE.Color(16724770);for(b=0;b<j.length-1;b+=1){var m=j[b].system,n=j[b+1].system,o=this.createRouteGeometry(m,n);if(o){c=new THREE.MeshBasicMaterial({color:k.clone().lerp(l,this.alphaOfSystem(n))});var p=new THREE.Mesh(o,c);p.position=m.sceneObject.position.clone(),p.lookAt(n.sceneObject.position),this._routeObject.add(p)}}if(0===j.length)return $("#routelist").append('<p class="no-route">No route set.</p>'),void 0;if("object"==typeof this.start.sceneObject){var q=window.map.createSelectorObject(k);for(q.scale.set(3.8,3.8,3.8),q.position.copy(this.start.sceneObject.position),q.visible=!0,this._routeObject.add(q),b=0;b<this.waypoints.length;b+=1)"object"==typeof this.waypoints[b].sceneObject&&(q=window.map.createSelectorObject(k.clone().lerp(l,this.alphaOfSystem(this.waypoints[b]))),q.scale.set(3.8,3.8,3.8),q.position.copy(this.waypoints[b].sceneObject.position),q.visible=!0,this._routeObject.add(q));scene.add(this._routeObject)}if($("#routelist").empty(),j.length>1){$("#routelist").append('<table class="routelist"><thead></thead><tbody></tbody></table>'),$("#routelist table").append("<caption>Route from "+j[0].system.createInfoLink(!0).outerHtml()+" to "+j[j.length-1].system.createInfoLink(!0).outerHtml()+' along <strong class="route-count"></strong> jump points:</caption>');var r=0;for(b=0;b<j.length;b+=1)d=j[b].system,b>0&&d.id===j[b-1].system.id?$("#routelist tr").last().addClass("waypoint").find("td.control i").addClass("fa-lg").addClass("fa-times").addClass("text-danger").removeClass("fa-long-arrow-down").prop("title","Remove waypoint").wrap('<a href="#" class="remove-waypoint" data-system="'+d.id+'"></a>'):(r+=1,f=1800,e=$('<tr class="waypoint"><th class="count muted">'+r+'</th><td class="control muted"></td><td class="system">'+d.createInfoLink(!1,!0).outerHtml()+'</td><td class="duration muted small"></td></tr>'),0===b?(f=900,e.addClass("start").find("td.control").html('<i class="fa fa-fw fa-lg fa-flag" title="Start"></i>')):b===j.length-1?(f=900,e.addClass("end").find("td.control").html('<i class="fa fa-fw fa-lg fa-flag-checkered" title="Destination"></i>')):e.find("td.control").html('<i class="fa fa-fw fa-lg fa-long-arrow-down" title="Destination"></i>'),e.find("td.duration").html("&plusmn;"+f.toHMM()),g+=f,$("#routelist tbody").append(e));$("#routelist table").append('<tfoot><tr><th class="count">&nbsp;</th><th class="control">&nbsp;</th><th class="system">&nbsp;</th><th class="duration small">&plusmn;'+g.toHMM()+"</th></tr></tfoot>"),$("#routelist .route-count").text(r-1),$("#routelist").append('<p><button class="delete-route" id="delete-route"><i class="fa fa-fw fa-trash-o"></i>Delete route</button></p>')}else $("#routelist").append('<p class="impossible">No route available with your current settings</p>');this.toString()!==h&&($("#map_ui").data("jsp").reinitialise(),$("#map_ui").tabs("option","active",3))},createRouteGeometry:function(a,b){if(a.sceneObject&&b.sceneObject){var c=a.sceneObject.position.distanceTo(b.sceneObject.position),d=new THREE.CylinderGeometry(.6,.6,c,8,1,!0);return d.applyMatrix((new THREE.Matrix4).makeTranslation(0,c/2,0)),d.applyMatrix((new THREE.Matrix4).makeRotationX(THREE.Math.degToRad(90))),d}}},RouteSegmentFailed.prototype=new Error,RouteSegmentFailed.prototype.constructor=RouteSegmentFailed,SCMAP.Map=function(a){this.name="Star Citizen Persistent Universe",this.scene=a,this.canEdit=!1,$("#map_ui li.editor").hide(),this._interactables=[],this._route=null,this._selectorObject=this.createSelectorObject(10092441),a.add(this._selectorObject),this._mouseOverObject=this.createSelectorObject(8930559),this._mouseOverObject.scale.set(4,4,4),a.add(this._mouseOverObject),SCMAP.Faction.preprocessFactions(SCMAP.data.factions),SCMAP.Goods.preprocessGoods(SCMAP.data.goods),this.__currentlySelected=null;var b=this;$.ajax({url:$("#sc-map-config").data("systems-json"),async:!0,cache:!0,dataType:"json",ifModified:!0,timeout:5e3}).done(function(c){b.populate(c),a.add(b.buildReferenceGrid())}).fail(function(a,b,c){console.error("Ajax request failed:",c,b)})},SCMAP.Map.prototype={constructor:SCMAP.Map,getSelected:function(){return this.__currentlySelected},selected:function(){return this.getSelected()},setSelected:function(a){if(null!==a&&!(a instanceof SCMAP.System))throw new Error(a,"is not an instance of SCMAP.System");return this.__currentlySelected=a,a},createSelectorObject:function(a){var b=new THREE.Mesh(SCMAP.SelectedSystemGeometry,new THREE.MeshBasicMaterial({color:a}));return b.scale.set(4.2,4.2,4.2),b.visible=!1,b.userData.systemPosition=new THREE.Vector3(0,0,0),b.userData.isSelector=!0,b.userData.scaleY=function(a,b){var c=a.userData.systemPosition.y*(b/100);a.translateY(c-a.position.y)},b},__updateSelectorObject:function(a){a instanceof SCMAP.System?(this._selectorObject.visible=!0,this._selectorObject.userData.systemPosition.copy(a.position),this.moveSelectorTo(a),this.setSelected(a)):(this._selectorObject.visible=!1,this.setSelected(null))},route:function(){return this._route instanceof SCMAP.Route||(this._route=new SCMAP.Route,console.log("Created new route",this._route.toString())),this._route},setSelectionTo:function(a){return this.__updateSelectorObject(a)},clearSelection:function(){return this.__updateSelectorObject()},getSystemByName:function(a){return SCMAP.System.getByName(a)},interactables:function(){return this._interactables},deselect:function(){this.clearSelection(),$("#system-selected").hide(),$("#system-not-selected").show()},animateSelector:function(){var a=300*THREE.Math.degToRad(25e-5*Date.now());window.scene.traverse(function(b){b.userData.isSelector&&(b.rotation.y=a)})},updateSystems:function(){for(var a=0;a<SCMAP.System.List.length;a++)SCMAP.System.List[a].updateSceneObject(this.scene)},setAllLabelSizes:function(a){for(var b=0;b<SCMAP.System.List.length;b++)SCMAP.System.List[b].setLabelScale(a)},moveSelectorTo:function(a){var b,c,d,e,f,g,h=this,j=[];if(!(h._selectorObject.visible&&h.getSelected()instanceof SCMAP.System))return h._selectorObject.userData.systemPosition.copy(a.position),h._selectorObject.position.copy(a.sceneObject.position),h._selectorObject.visible=!0,h.getSelected(a),void 0;if(c=a.sceneObject.position.clone(),f=new SCMAP.Dijkstra(SCMAP.System.List,h.getSelected(),a),f.buildGraph(),g=f.routeArray(a),g.length<=1)return h._selectorObject.userData.systemPosition.copy(a.position),h._selectorObject.position.copy(a.sceneObject.position),h._selectorObject.visible=!0,h.setSelected(a),void 0;for(d={x:h._selectorObject.position.x,y:h._selectorObject.position.y,z:h._selectorObject.position.z},i=0;i<g.length-1;i++)e=g[i+1].system,b=new TWEEN.Tween(d).to({x:e.sceneObject.position.x,y:e.sceneObject.position.y,z:e.sceneObject.position.z},800/(g.length-1)).easing(TWEEN.Easing.Linear.None).onUpdate(function(){h._selectorObject.position.set(this.x,this.y,this.z)}),0==i&&(2==g.length?b.easing(TWEEN.Easing.Cubic.InOut):b.easing(TWEEN.Easing.Cubic.In)),i>0&&j[i-1].chain(b),i==g.length-2&&(b.easing(TWEEN.Easing.Cubic.Out),b.onComplete(function(){h._selectorObject.userData.systemPosition.copy(e.position),h._selectorObject.position.copy(e.sceneObject.position),h.setSelected(a)})),j.push(b);j[0].start()},populate:function(a){var b,c,d,e,f=0,g=this;c=d=new Date,SCMAP.System.preprocessSystems(a),$(SCMAP.System.List).each(function(a,b){sceneObject=b.buildSceneObject(),g.scene.add(sceneObject),g._interactables.push(sceneObject.children[0]),f++}),$(SCMAP.System.List).each(function(a,c){for(e=0;e<c.jumpPoints.length;e++)b=c.jumpPoints[e].buildSceneObject(),b instanceof THREE.Object3D&&(c._routeObjects.push(b),g.scene.add(b))}),c=new Date,console.log("Populating the scene (without ref plane) took "+(c.getTime()-d.getTime())+" msec"),$("#debug-systems").html(f+" systems loaded");var h=this;SCMAP.UI.waitForFontAwesome(function(){h.updateSystems()})},closestPOI:function(a){var b,c,d,e,f,g,h=1/0;for(e in SCMAP.data.systems)c=SCMAP.System.getByName(e),f=a.x-c.position.x,g=a.z-c.position.z,d=Math.sqrt(f*f+g*g),h>d&&(h=d,b=c);return[h,b]},closestFromArray:function(a,b){for(var c,d,e,f,g,h=1/0,i=0;i<b.length;i++)d=b[i],f=a.x-d.position.x,g=a.z-d.position.z,e=Math.sqrt(f*f+g*g),h>e&&(h=e,c=d);return[h,c]},withinApproxDistance:function(a,b){for(var c=[],d=0;d<SCMAP.System.List.length;d+=1){var e=SCMAP.System.List[d];e.position.x<a.x-b||e.position.x>a.x+b||e.position.z<a.z-b||e.position.z>a.z+b||c.push(e)}return c},furthestPOI:function(a){var b,c,d,e,f,g,h=0;for(e in SCMAP.data.systems)c=SCMAP.System.getByName[e],f=a.x-c.position.x,g=a.z-c.position.z,d=Math.sqrt(f*f+g*g),d>h&&(h=d,b=c);return[h,b]},pointAtPlane:function(a,b,c){return new THREE.Vector3(b*Math.cos(a),c,-b*Math.sin(a))},buildReferenceGrid:function(){function a(a,b,c,d){if(G>=65534)throw new Error("Too many points");return F.push(a[0],a[1],a[2]),H.push(b.r,b.g,b.b),G++,F.push(c[0],c[1],c[2]),H.push(d.r,d.g,d.b),I.push(G-1,G),G++}var b,c,d,e,f,g,h,i,j,k,l,m,n,o,p,q=10,r=0,s=0,t=0,u=0,v={},w=new THREE.BufferGeometry,x={},y={};for(h=i=new Date,b=0;b<SCMAP.System.List.length;b+=1)g=SCMAP.System.List[b].position,g.x<r&&(r=g.x-60),g.x>t&&(t=g.x+80),g.z<s&&(s=g.z-60),g.z>u&&(u=g.z+100);r=Math.floor(r/q)*q,s=Math.floor(s/q)*q,t=Math.floor(t/q)*q,u=Math.floor(u/q)*q;for(var z=s;u>=z;z+=q){x[z]={};for(var A=r;t>=A;A+=q){y[A]=!0;var B=new THREE.Vector3(A,0,z),C=this.withinApproxDistance(B,6.5*q);p=this.colorForVector(B,C,q),p!==SCMAP.Map.BLACK?(x[z][A]=p.getHexString(),void 0===v[x[z][A]]&&(v[x[z][A]]=p)):(x[z][A]=null,v[null]=SCMAP.Map.BLACK)}}var D=[];for(c in y)D.push(c);D.sort(function(a,b){return a-b}),y=D;var E=[];for(c in x)E.push(c);E.sort(function(a,b){return a-b});var F=[],G=0,H=[],I=[];for(b=1;b<E.length;b+=1){for(f=E[b],n=[],o=[],c=1;c<y.length;c+=1)e=y[c],j=Math.floor(Number(e)-q),k=Math.floor(Number(e)+q),vertexColor=x[f][e],(vertexColor!==x[f][j]&&x[f][j]||vertexColor!==x[f][k]&&x[f][k])&&(n.push([e,0,f]),o.push(v[vertexColor]));for(d=0;d<n.length-1;d++)a(n[d],o[d],n[d+1],o[d+1])}for(b=1;b<y.length;b+=1){for(e=y[b],n=[],o=[],c=1;c<E.length;c+=1)f=E[c],l=Math.floor(Number(f)-q),m=Math.floor(Number(f)+q),vertexColor=x[f][e],(x[l]&&x[l][e]&&vertexColor!==x[l][e]||x[m]&&x[m][e]&&vertexColor!==x[m][e])&&(n.push([e,0,f]),o.push(v[vertexColor]));for(d=0;d<n.length-1;d++)a(n[d],o[d],n[d+1],o[d+1])}var J=new THREE.BufferAttribute;J.array=new Uint16Array(I),J.itemSize=1,w.addAttribute("index",J),J=new THREE.BufferAttribute,J.array=new Float32Array(F),J.itemSize=3,w.addAttribute("position",J),J=new THREE.BufferAttribute,J.array=new Float32Array(H),J.itemSize=3,w.addAttribute("color",J),w.dynamic=!1,w.computeBoundingBox();var K=new THREE.Line(w,new THREE.LineBasicMaterial({linewidth:1.5,wireframe:!0,vertexColors:THREE.VertexColors}),THREE.LinePieces);return K.matrixAutoUpdate=!1,h=new Date,console.log("Building the grid reference plane took "+(h.getTime()-i.getTime())+" msec"),K},colorForVector:function(a,b,c){var d=SCMAP.Map.BLACK,e=this.closestFromArray(a,b);return e[0]<=4.5*c&&e[1]&&(d=e[1].faction.planeColor.clone(),e[0]>=4*c?d.multiplyScalar(.5):e[0]>=3*c&&d.multiplyScalar(.8)),d}},SCMAP.Map.BLACK=new THREE.Color(0),SCMAP.UI=function(){if($("#home")){$("#home").append(SCMAP.UI.Templates.main.instructions({items:["Left-click and release to select a system.","Left-click and drag from system to system to map a route between them.","Left-click and drag any waypoint on the route to move it. It moves an existing waypoint or creates new waypoints as needed.","Left-click and drag on the map to rotate the camera around the center of the map.","Mousewheel to zoom in and out, middle-click and drag can also be used.","Right-click to pan the camera around the map."]})),$("#home").append(SCMAP.UI.Templates.main.shortcuts({items:[{key:"R",description:"Reset camera orientation"},{key:"C",description:"Camera to center (Sol)"},{key:"T",description:"Top-down camera"},{key:"L",description:"Lock/unlock camera rotation"},{key:"2",description:"Switch to 2D mode"},{key:"3",description:"Switch to 3D mode"},{key:"Esc",description:"Deselect target"}]}));var a=[];for(var b in SCMAP.Symbols)b=SCMAP.Symbols[b],a.push($('<span><i class="fa-li fa '+b.faClass+'"></i>'+b.description+"</span>").css("color",b.color).outerHtml());$("#home").append(SCMAP.UI.Templates.main.iconLegend({items:a})),$("#home").append(SCMAP.UI.Templates.main.quickFunctions({})),$("#3d-mode").prop("checked","3d"===SCMAP.settings.mode),$("#3d-mode").on("change",function(){this.checked?displayState.to3d():displayState.to2d()}),$("#lock-rotation").prop("checked",SCMAP.settings.control.rotationLocked),$("#lock-rotation").on("change",function(){controls.noRotate=this.checked,storage&&(storage["control.rotationLocked"]=this.checked?"1":"0")}),$("#resetCamera").on("click",function(){controls.cameraTo(SCMAP.settings.cameraDefaults.target,SCMAP.settings.cameraDefaults.orientation.theta,SCMAP.settings.cameraDefaults.orientation.phi,SCMAP.settings.cameraDefaults.orientation.radius)}),$("#centreCamera").on("click",function(){controls.moveTo(SCMAP.settings.cameraDefaults.target)}),$("#northCamera").on("click",function(){controls.rotateTo(0,void 0,void 0)}),$("#topCamera").on("click",function(){controls.rotateTo(0,0,180)}),$("#top2D").on("click",function(){controls.noRotate=!0,$("#lock-rotation").prop("checked",!0),displayState.to2d(),controls.rotateTo(0,0,180)})}$("#route")&&$("#route").append(SCMAP.UI.Templates.routeList({})),$("#map_ui").tabs({active:0,activate:function(a,b){a.preventDefault();var c=b.newTab.find("a").attr("href");switch(c){case"#editor":map.canEdit&&$("#webgl-container").removeClass().addClass("noselect webgl-container-edit");break;case"#listing":SCMAP.UI.buildDynamicLists();break;default:$("#webgl-container").removeClass().addClass("noselect webgl-container-noedit")}$("#map_ui").data("jsp").reinitialise()}}),$("#map_ui").jScrollPane({showArrows:!0}),$("#toggle-glow").prop("checked",SCMAP.settings.glow),$("#toggle-labels").prop("checked",SCMAP.settings.labels),$("#toggle-label-icons").prop("checked",SCMAP.settings.labelIcons),$("#avoid-hostile").prop("checked",SCMAP.settings.route.avoidHostile),$("#avoid-off-limits").prop("checked",SCMAP.settings.route.avoidOffLimits),$("#avoid-unknown-jumppoints").prop("checked",SCMAP.settings.route.avoidUnknownJumppoints),$("#avoid-hostile").on("change",function(){SCMAP.settings.route.avoidHostile=this.checked,SCMAP.settings.save("route"),map.route().rebuildCurrentRoute()}),$("#avoid-off-limits").on("change",function(){SCMAP.settings.route.avoidOffLimits=this.checked,SCMAP.settings.save("route"),map.route().rebuildCurrentRoute()}),$("#avoid-unknown-jumppoints").on("change",function(){SCMAP.settings.route.avoidUnknownJumppoints=this.checked,SCMAP.settings.save("route"),map.route().rebuildCurrentRoute()}),$("#toggle-stats").prop("checked",storage&&"1"===storage["renderer.Stats"]?!0:!1).on("change",function(){this.checked?$("#stats").show():$("#stats").hide(),storage&&(storage["renderer.Stats"]=this.checked?"1":"0")}),$("#toggle-antialias").prop("checked",SCMAP.settings.effect.Antialias).on("change",function(){SCMAP.settings.effect.Antialias=this.checked,SCMAP.settings.save("effect"),console.log(localStorage.effect),window.location.reload(!1)}),$("#toggle-fxaa").prop("checked",SCMAP.settings.effect.FXAA).prop("disabled",SCMAP.settings.effect.Antialias).on("change",function(){SCMAP.settings.effect.FXAA=this.checked,SCMAP.settings.save("effect"),effectFXAA&&(effectFXAA.enabled=this.checked)}),$("#toggle-bloom").prop("checked",SCMAP.settings.effect.Bloom).prop("disabled",SCMAP.settings.effect.Antialias).on("change",function(){if(SCMAP.settings.effect.Bloom=this.checked,SCMAP.settings.save("effect"),composer)for(var a=0;a<composer.passes.length;a++)composer.passes[a]instanceof THREE.BloomPass&&(composer.passes[a].enabled=this.checked)}),$("#toggle-glow").on("change",function(){SCMAP.settings.glow=this.checked,map.updateSystems(),storage&&(storage["settings.Glow"]=this.checked?"1":"0")}),$("#toggle-labels").on("change",function(){SCMAP.settings.labels=this.checked,$("#toggle-label-icons").prop("disabled",!SCMAP.settings.labels),map.updateSystems(),storage&&(storage["settings.Labels"]=this.checked?"1":"0")}),$("#toggle-label-icons").prop("disabled",!SCMAP.settings.labels).on("change",function(){SCMAP.settings.labelIcons=this.checked,map.updateSystems(),storage&&(storage["settings.LabelIcons"]=this.checked?"1":"0")}),$(".quick-button.with-checkbox").on("click",function(){var a=$(this);a.find("input[type=checkbox]").click()}),$("#map_ui").on("click","a[data-toggle-next]",function(a){var b=$(this);a.preventDefault();var c=b.parent().next();c.toggle(),c.is(":visible")?b.parent().find("> a > i").first().removeClass("fa-caret-right").addClass("fa-caret-down"):b.parent().find("> a > i").first().addClass("fa-caret-right").removeClass("fa-caret-down"),$("#map_ui").data("jsp").reinitialise()}),$("#map_ui").on("click","a[data-toggle-child]",function(a){var b=$(this);a.preventDefault();var c=b.parent().find(b.data("toggle-child"));c.toggle(),c.is(":visible")?b.parent().find("> a > i").removeClass("fa-caret-right").addClass("fa-caret-down"):b.parent().find("> a > i").addClass("fa-caret-right").removeClass("fa-caret-down"),$("#map_ui").data("jsp").reinitialise()}),$("#map_ui").on("click","a[data-goto='system']",function(a){a.preventDefault();var b=$(this),c=SCMAP.System.getById(b.data("system"));c.displayInfo(),controls.moveTo(c)}),$("#map_ui #routelist").on("click","td.control a.remove-waypoint",function(a){a.preventDefault();var b=$(this),c=SCMAP.System.getById(b.data("system"));map.route().removeWaypoint(c),map.route().update()}),$("#map_ui #routelist").on("click","button.delete-route",function(){map.route().destroy()});var c=function(a){a.preventDefault();var b=SCMAP.System.getById($(this).data("system")),c=$(this).val();"string"==typeof c&&c.length>0?(b.setComments(c),$("#map_ui .user-system-comments-md").html($(markdown.toHTML(c)))):(b.setComments(),$("#map_ui .user-system-comments-md").empty()),b.updateSceneObject(scene)};$("#map_ui").on("keyup",".user-system-comments",c),$("#map_ui").on("blur",".user-system-comments",c),$("#map_ui").on("change",".user-system-comments",c),$("#map_ui").on("click",".remove-system-comments",function(a){a.preventDefault();var b=SCMAP.System.getById($(this).data("system"));b.setComments(),$("#comments").empty().val(""),$("#comments-md").empty(),b.updateSceneObject(scene)}),$("#map_ui").on("change",".user-system-bookmarked",function(){SCMAP.System.getById($(this).data("system")).setBookmarkedState(this.checked).updateSceneObject(scene),SCMAP.settings.save("systems")}),$("#map_ui").on("change",".user-system-ishangar",function(){SCMAP.System.getById($(this).data("system")).setHangarState(this.checked).updateSceneObject(scene),SCMAP.settings.save("systems")}),$("#map_ui").on("change",".user-system-avoid",function(){SCMAP.System.getById($(this).data("system")).setToBeAvoidedState(this.checked).updateSceneObject(scene),SCMAP.settings.save("systems"),map.route().rebuildCurrentRoute()}),$("#map_ui a[href='#']").removeAttr("href")},SCMAP.UI.prototype={constructor:SCMAP.UI,toTabIndex:function(){window.ui.updateHeight()},updateHeight:function(){}},SCMAP.UI.makeSafeForCSS=function(a){return"string"==typeof a?a.replace(/[^a-zA-Z0-9]/g,function(a){var b=a.charCodeAt(0);return 32==b?"-":b>=65&&90>=b?"_"+a.toLowerCase():b.toString(16).slice(-4)}):void 0},SCMAP.UI.fontAwesomeIsReady=!1,SCMAP.UI.waitForFontAwesome=function(a){function b(){var d,e;c-=1,d=document.createElement("canvas"),d.width=20,d.height=20,e=d.getContext("2d"),e.font="16pt FontAwesome",e.textAlign="center",e.fillStyle="rgba(255,255,255,1.0)",e.fillText("",10,18);var f=e.getImageData(10,10,1,1).data;0===f[0]&&0===f[1]&&0===f[2]?(console.log("FontAwesome is not yet available, retrying ..."),c>0&&setTimeout(b,200)):(console.log("FontAwesome is loaded"),SCMAP.UI.fontAwesomeIsReady=!0,"function"==typeof a&&a())}var c=5;b()},SCMAP.UI.buildDynamicLists=function(){var a=[],b=[],c=[],d=[];$(SCMAP.System.List).each(function(e,f){var g=f.createInfoLink().outerHtml();f.hasHangar()&&a.push(g),f.isBookmarked()&&b.push(g),f.hasComments()&&c.push(g),d.push(g)}),$("#listing").empty().append(SCMAP.UI.Templates.listings({sections:[{title:"Hangar locations&nbsp;"+SCMAP.Symbol.getTag(SCMAP.Symbols.HANGAR).addClass("fa-lg").outerHtml(),items:a},{title:"Bookmarked&nbsp;"+SCMAP.Symbol.getTag(SCMAP.Symbols.BOOKMARK).addClass("fa-lg").outerHtml(),items:b},{title:"With your comments&nbsp;"+SCMAP.Symbol.getTag(SCMAP.Symbols.COMMENTS).addClass("fa-lg").outerHtml(),items:c},{title:"Everything",items:d}]}))};var sectionLevel=1;Handlebars.registerHelper("uiSection",function(a,b,c){var d="",e=b?!0:!1,f="fa-caret-right",g='style="display: none;"';"class"in c.hash&&(d='class="'+c.hash.class+'"'),e&&(f="fa-caret-down",g="");var h=sectionLevel++;return new Handlebars.SafeString("<h"+h+'><a href="#" data-toggle-next="next" '+d+'><i class="fa fa-fw fa-lg '+f+'"></i>'+a+"</a></h"+h+'><div class="ui-section" '+g+">")}),Handlebars.registerHelper("endUiSection",function(){return sectionLevel-=1,1>sectionLevel&&(sectionLevel=1),new Handlebars.SafeString("</div>")}),Handlebars.registerHelper("tabHeader",function(a){return new Handlebars.SafeString('<h1 class="padleft">'+a+"</h1>")}),Handlebars.registerHelper("bigButton",function(a,b,c){return new Handlebars.SafeString('<button class="big-button" id="'+a+'"><i class="fa '+b+' fa-fw fa-lg"></i> '+c+"</button><br>")}),Handlebars.registerHelper("commoditiesList",function(a){return a.length?new Handlebars.SafeString($.map(a,function(a){return SCMAP.data.goods[a].name}).join(", ")):new Handlebars.SafeString("&mdash;")}),Handlebars.registerHelper("markdown",function(a){return new Handlebars.SafeString(markdown.toHTML(a))}),Handlebars.registerHelper("colourGetStyle",function(a){return new Handlebars.SafeString(a.getStyle())}),Handlebars.registerHelper("systemLink",function(a,b){console.log(b);var c=!1,d=!1;return"noIcons"in b.hash&&(c=b.hash.noIcons?!0:!1),"noTarget"in b.hash&&(d=b.hash.noTarget?!0:!1),new Handlebars.SafeString(a.createInfoLink(c,d).outerHtml())}),Handlebars.registerHelper("checkboxButton",function(a,b,c){var d=[];for(var e in c.hash)"icon"===e?b=b+' <i class="fa fa-lg fa-fw '+c.hash[e]+'"></i>':d.push(e+'="'+c.hash[e]+'"');return new Handlebars.SafeString('<span class="checkmark-button"><input type="checkbox" id="'+a+'" '+d.join(" ")+'><label for="'+a+'">'+b+"</label></span>")}),Handlebars.registerHelper("checkboxOption",function(a,b,c,d){var e=[];for(var f in d.hash)"icon"===f?b=b+' <i class="fa fa-lg fa-fw '+d.hash[f]+'"></i>':e.push(f+'="'+d.hash[f]+'"');return new Handlebars.SafeString('<span class="checkmark-option"><input type="checkbox" id="'+a+'"><label for="'+a+'">'+b+'<span class="small label-info">'+c+"</span></label></span>")}),Handlebars.registerPartial("faUL",$("#template-fa-UL").html()),SCMAP.UI.Templates={basicDL:Handlebars.compile($("#template-basicDL").html()),faUL:Handlebars.compile($("#template-fa-UL").html()),systemInfo:Handlebars.compile($("#template-system-info").html()),main:{instructions:Handlebars.compile($("#template-main-instructions").html()),shortcuts:Handlebars.compile($("#template-main-shortcuts").html()),iconLegend:Handlebars.compile($("#template-main-icon-legend").html()),quickFunctions:Handlebars.compile($("#template-main-quick-functions").html())},listings:Handlebars.compile($("#template-systems-listing").html()),routeList:Handlebars.compile($("#template-route-list").html())},SCMAP.OrbitControls=function(a,b){function c(){return Math.pow(.95,t.zoomSpeed)}function d(a){t.enabled!==!1&&(t.requireAlt!==!0||a.altKey!==!1)&&(a.preventDefault(),v.starttouch(a),t.domElement.addEventListener("mouseup",f,!1))}function e(a){if(t.enabled!==!1&&(t.requireAlt!==!0||a.altKey!==!1)){a.preventDefault();var b,c,d=t.domElement===document?t.domElement.body:t.domElement;if(t.cameraIsMoving()||a.clientX!==G.x&&a.clientY!==G.y&&(b=t.getIntersect(a),b&&b.object.parent.userData.system&&b.object.parent.userData.system!==n?(n=b.object.parent.userData.system,window.map._mouseOverObject.position.copy(n.sceneObject.position),window.map._mouseOverObject.visible=!0):b&&b.object.parent.userData.system||(void 0!==n&&(window.map._mouseOverObject.position.set(0,0,0),window.map._mouseOverObject.visible=!1),n=void 0)),G.set(a.clientX,a.clientY),"touch"===v.current)0===a.button?v.touchtorotate(a):1===a.button?v.touchtodolly(a):2===a.button&&v.touchtopan(a);else if("drag"===v.current)l&&b&&b.object.parent.userData.system&&b.object.parent.userData.system!==l&&(m&&m===b.object.parent.userData.system?m=void 0:(m=b.object.parent.userData.system,c=window.map.route(),c.isSet()?(c.moveWaypoint(l,m),l!==m&&c.update(),l=m,console.log('In-route mode -- intermediate object while dragging is "'+m.name+'"')):(c.start=l,c.waypoints=[m],c.update(m),t.debug&&console.log('Intermediate object while dragging is "'+m.name+'"'))));else if("rotate"===v.current){if(t.noRotate===!0)return;y.set(a.clientX,a.clientY),z.subVectors(y,x),t.rotateLeft(2*Math.PI*z.x/d.clientWidth*t.rotateSpeed),t.rotateUp(2*Math.PI*z.y/d.clientHeight*t.rotateSpeed),x.copy(y)}else if("dolly"===v.current){if(t.noZoom===!0)return;E.set(a.clientX,a.clientY),F.subVectors(E,D),F.y>0?t.dollyIn():t.dollyOut(),D.copy(E)}else if("pan"===v.current){if(t.noPan===!0)return;B.set(a.clientX,a.clientY),C.subVectors(B,A),t.pan(C),A.copy(B)}t.update()}}function f(a){t.enabled!==!1&&(t.requireAlt!==!0||a.altKey!==!1)&&(t.domElement.removeEventListener("mouseup",f,!1),v.idle(a))}function g(a){if(t.enabled!==!1&&t.noZoom!==!0&&(t.requireAlt!==!0||a.altKey!==!1)){var b=0;a.deltaY?b=a.deltaY:a.wheelDelta?b=a.wheelDelta:a.detail&&(b=-a.detail),b>0?t.dollyOut():0>b&&t.dollyIn()}}function h(a){if(t.enabled!==!1&&t.noKeys!==!0&&t.noPan!==!0&&(t.requireAlt!==!0||a.altKey!==!1)&&a.altKey!==!0&&a.shiftKey!==!0&&a.ctrlKey!==!0&&a.metaKey!==!0){var b=$(document.activeElement);if("comments"===b.attr("id"))return(a.keyCode==t.keys.ESCAPE||a.keyCode==t.keys.TAB)&&b.blur(),void 0;var c=!1;switch(a.keyCode){case t.keys.UP:a.preventDefault(),t.pan(new THREE.Vector2(0,t.keyPanSpeed)),c=!0;break;case t.keys.BOTTOM:a.preventDefault(),t.pan(new THREE.Vector2(0,-t.keyPanSpeed)),c=!0;break;case t.keys.LEFT:a.preventDefault(),t.pan(new THREE.Vector2(t.keyPanSpeed,0)),c=!0;break;case t.keys.RIGHT:a.preventDefault(),t.pan(new THREE.Vector2(-t.keyPanSpeed,0)),c=!0;break;case t.keys.ESCAPE:window.map.deselect();break;case t.keys.TAB:break;case t.keys.R:t.rotateTo(0,void 0,void 0);break;case t.keys.C:t.moveTo(SCMAP.settings.camera.target);break;case t.keys.T:t.rotateTo(0,0,200);break;case t.keys["2"]:t.noRotate=!0,$("#lock-rotation").prop("checked",!0),displayState.to2d(),t.rotateTo(0,0,180);break;case t.keys["3"]:displayState.to3d(),t.rotateTo(SCMAP.settings.cameraDefaults.orientation.theta,SCMAP.settings.cameraDefaults.orientation.phi,SCMAP.settings.cameraDefaults.orientation.radius);break;case t.keys.L:$("#lock-rotation").click()}c&&(M=!0,t.update())}}function i(a){t.enabled!==!1&&t.requireAlt===!0&&a.altKey===!1}function j(a){t.enabled!==!1&&(t.requireAlt!==!0||a.altKey!==!1)&&(a.preventDefault(),a.stopPropagation())}function k(a){t.enabled!==!1&&v.idle(a)}this.object=a,this.domElement=void 0!==b?b:document,this.enabled=!0,this.target=new THREE.Vector3,this.center=this.target,this.noZoom=!1,this.zoomSpeed=1,this.minDistance=20,this.maxDistance=800,this.noRotate=!1,this.rotateSpeed=1,this.noPan=!1,this.keyPanSpeed=40,this.mapMode=!0,this.requireAlt=!1,this.minPolarAngle=0,this.maxPolarAngle=THREE.Math.degToRad(85),this.noKeys=!1,this.keys={LEFT:37,UP:38,RIGHT:39,BOTTOM:40,ESCAPE:27,R:82,C:67,T:84,2:50,3:51,L:76,SPACE:32,TAB:9},this.debug=!1;var l,m,n,o,p,q,r,s,t=this,u="15.0",v=StateMachine.create({initial:{state:"loading",event:"init",defer:!0},events:[{name:"startup",from:"loading",to:"idle"},{name:"starttouch",from:"idle",to:"touch"},{name:"touchtodrag",from:"touch",to:"drag"},{name:"touchtorotate",from:"touch",to:"rotate"},{name:"touchtodolly",from:"touch",to:"dolly"},{name:"touchtopan",from:"touch",to:"pan"},{name:"startdolly",from:"idle",to:"dolly"},{name:"startpan",from:"idle",to:"pan"},{name:"idle",from:["*"],to:"idle"}],callbacks:{onenterstate:function(a,b,c){t.showState(c)
},onentertouch:function(a,b,c,d){if(t.enabled!==!1&&(t.requireAlt!==!0||d.altKey!==!1)){d.preventDefault(),t.debug&&console.log(a,": entered state",c,"from",b);var e;if(0===d.button){if(e=t.getIntersect(d),e&&e.object.parent.userData.system)l=e.object.parent.userData.system,t.debug&&console.log('Click at "'+e.object.parent.userData.system.name+'"'),window.map.setSelectionTo(l),l.displayInfo("doNotSwitch"),this.touchtodrag(d);else if(!t.noRotate){if(t.noRotate===!0)return;x.set(d.clientX,d.clientY),this.touchtorotate(d)}}else if(1===d.button){if(t.noZoom===!0)return;D.set(d.clientX,d.clientY),this.touchtodolly(d)}if(2===d.button||t.noRotate&&!e){if(t.noPan===!0)return;A.set(d.clientX,d.clientY),this.touchtopan(d)}}},onenterdrag:function(a,b,c){t.enabled!==!1&&t.debug&&console.log(a,": entered state",c,"from",b)},onenteridle:function(a,b,c,d){if(t.debug&&console.log(a,": idling after",b),"drag"===b){var e=t.getIntersect(d);if(e&&e.object.parent.userData.system)if(m=e.object.parent.userData.system,t.debug&&console.log('Ended dragging at "'+m.toString()+'"'),m===l)$("#map_ui").tabs("option","active",2),m.displayInfo();else{window.map.selected()===m;var f=window.map.route();f.isSet()&&l!==m&&(f.update(m),$("#map_ui").tabs("option","active",3))}}l=void 0,m=void 0,t.debug&&console.log("idling ...")}}}),w=1e-6,x=new THREE.Vector2,y=new THREE.Vector2,z=new THREE.Vector2,A=new THREE.Vector2,B=new THREE.Vector2,C=new THREE.Vector2,D=new THREE.Vector2,E=new THREE.Vector2,F=new THREE.Vector2,G=new THREE.Vector2,H=0,I=0,J=1,K=new THREE.Vector3,L=new THREE.Vector3,M=!1,N={type:"change"};this.stateMachine=function(){return v},this.showState=function(a){window.jQuery&&window.jQuery("#debug-state")&&window.jQuery("#debug-state").text("State: "+a)},this.rotateLeft=function(a){void 0===a&&(a=getAutoRotationAngle()),I-=a},this.rotateUp=function(a){void 0===a&&(a=getAutoRotationAngle()),H-=a},this.panLeft=function(a){var b=new THREE.Vector3,c=this.object.matrix.elements;b.set(c[0],c[1],c[2]),b.multiplyScalar(-a),K.add(b)},this.panUp=function(a){if(this.mapMode)return this.panBack(a);var b=new THREE.Vector3,c=this.object.matrix.elements;b.set(c[4],c[5],c[6]),b.multiplyScalar(a),K.add(b)},this.panBack=function(a){var b=this.target.clone().setY(this.object.position.y),c=this.object.position.clone().sub(b).negate().setLength(a);K.add(c)},this.moveTo=function(a){var b,c=this,d=this.target.clone();if(a instanceof SCMAP.System)b=a.position.clone().setY(this.target.y);else{if(!(a instanceof THREE.Vector3))return;b=a.clone().setY(this.target.y)}a instanceof SCMAP.System&&window.map.setSelectionTo(a),o&&o.stop(),o=new TWEEN.Tween(d).to({x:b.x,y:b.y,z:b.z},750).easing(TWEEN.Easing.Cubic.InOut).onUpdate(function(){var a=new THREE.Vector3(this.x,this.y,this.z);c.goTo(a),M=!0}),o.onComplete(function(){o=void 0}),o.start()},this.rememberPosition=function(){SCMAP.settings.camera.camera=this.object.position,SCMAP.settings.camera.target=this.target,SCMAP.settings.save("camera")},this.rotateTo=function(a,b,c){var d=this.objectVectorToTarget(),e=Math.atan2(d.x,d.z),f=Math.atan2(Math.sqrt(d.x*d.x+d.z*d.z),d.y),g={left:e,up:f,radius:d.length()};rotateLeft=g.left,rotateUp=g.up,p&&p.stop(),p=new TWEEN.Tween(g).to({left:a,up:b,radius:c},1e3).easing(TWEEN.Easing.Cubic.InOut).onUpdate(function(){q=this.left,r=this.up,s=this.radius,M=!0}),p.onComplete(function(){p=void 0,q=void 0,r=void 0,s=void 0}),p.start()},this.cameraTo=function(a,b,c,d){this.rotateTo(b,c,d),this.moveTo(a)},this.goTo=function(a){a=a.clone().setY(this.target.y),a.sub(this.target),K.add(a)},this.pan=function(a){var b=t.domElement===document?t.domElement.body:t.domElement;if(void 0!==t.object.fov){var c=t.objectVectorToTarget().length();c*=Math.tan(t.object.fov/2*Math.PI/180),t.panLeft(2*a.x*c/b.clientHeight),t.panUp(2*a.y*c/b.clientHeight)}else void 0!==t.object.top?(t.panLeft(a.x*(t.object.right-t.object.left)/b.clientWidth),t.panUp(a.y*(t.object.top-t.object.bottom)/b.clientHeight)):console.warn("WARNING: OrbitControls.js encountered an unknown camera type - pan disabled.")},this.dollyIn=function(a){void 0===a&&(a=c()),J/=a,M=!0},this.dollyOut=function(a){void 0===a&&(a=c()),J*=a,M=!0},this.getIntersect=function(a){if(window.map.interactables()){var b,c,d,e;return b=new THREE.Vector3(a.clientX/window.innerWidth*2-1,2*-(a.clientY/window.innerHeight)+1,.5),c=new THREE.Projector,c.unprojectVector(b,t.object),d=new THREE.Raycaster(t.object.position,b.sub(t.object.position).normalize()),e=d.intersectObjects(window.map.interactables()),e[0]}},this.objectVectorToTarget=function(){return this.object.position.clone().sub(this.target)},this.currentState=function(){return v.current},this.cameraIsMoving=function(){return"pan"===v.current||"dolly"===v.current||"rotate"===v.current||"loading"===v.current||!D.equals(E)||M},this.update=function(){if("idle"!==v.current||0!==K.length()||M){M=!0;var a=this.objectVectorToTarget();this.target.add(K);var b=Math.atan2(a.x,a.z),c=Math.atan2(Math.sqrt(a.x*a.x+a.z*a.z),a.y);b+=I,c+=H,void 0!==q&&(b=q),void 0!==r&&(c=r),c=Math.max(this.minPolarAngle,Math.min(this.maxPolarAngle,c)),c=Math.max(w,Math.min(Math.PI-w,c));var d=a.length()*J;void 0!==s&&(d=s),$("#debug-angle").html("Camera heading: "+THREE.Math.radToDeg(b).toFixed(1)+"&deg;<br>Camera tilt: "+THREE.Math.radToDeg(c).toFixed(1)+"&deg;<br>Camera distance: "+d.toFixed(1));var e=d-20;e/=10,17>e?e=17:e>22&&(e=22),e.toFixed(1)!==u&&(window.map.setAllLabelSizes(new THREE.Vector3(e,e,1)),u=e.toFixed(1)),d=Math.max(this.minDistance,Math.min(this.maxDistance,d)),a.x=d*Math.sin(c)*Math.sin(b),a.y=d*Math.cos(c),a.z=d*Math.sin(c)*Math.cos(b),this.object.position.copy(this.target).add(a),this.object.lookAt(this.target),I=0,H=0,J=1,K.set(0,0,0),L.distanceTo(this.object.position)>0&&(this.dispatchEvent(N),L.copy(this.object.position)),this.rememberPosition(),M=!1}},v.init(),this.domElement.addEventListener("contextmenu",function(a){a.preventDefault()},!1),this.domElement.addEventListener("mousedown",d,!1),this.domElement.addEventListener("mousewheel",g,!1),this.domElement.addEventListener("DOMMouseScroll",g,!1),$(this.domElement).on("mousewheel",g),$(this.domElement).on("mouseenter",function(a){v.idle(a)}),window.addEventListener("keydown",h,!1),this.domElement.addEventListener("touchstart",i,!1),this.domElement.addEventListener("touchend",k,!1),this.domElement.addEventListener("touchmove",j,!1),this.domElement.addEventListener("mousemove",e,!1)},SCMAP.OrbitControls.prototype=Object.create(THREE.EventDispatcher.prototype);var effectFXAA,camera,scene,renderer,composer,map,dpr,shift,ctrl,alt,controls,editor,stats,displayState,ui,storage;$(function(){Detector.webgl||Detector.addGetWebGLMessage(),init(),animate()}),Object.values=function(a){var b=[];for(var c in a)a.hasOwnProperty(c)&&b.push(a[c]);return b},jQuery.fn.outerHtml=function(){return jQuery("<div />").append(this.eq(0).clone()).html()},String.prototype.toHMM=function(){var a=parseInt(this,10),b=Math.floor(a/3600),c=Math.floor((a-3600*b)/60);10>c&&(c="0"+c);var d=b+":"+c;return d},Number.prototype.toHMM=function(){var a=parseInt(this,10),b=Math.floor(a/3600),c=Math.floor((a-3600*b)/60);10>c&&(c="0"+c);var d=b+":"+c;return d};