...
return a.geometry}function isInfinite(a){return!isFinite(a)}function init(){var a,b,c,d,e,f,g;hasLocalStorage()&&(localStorage=window.localStorage),dpr=1,void 0!==window.devicePixelRatio&&(dpr=window.devicePixelRatio),SCMAP.settings.glow=localStorage&&"0"===localStorage["settings.Glow"]?!1:!0,SCMAP.settings.labels=localStorage&&"0"===localStorage["settings.Labels"]?!1:!0,SCMAP.settings.labelIcons=localStorage&&"0"===localStorage["settings.LabelIcons"]?!1:!0,$("#toggle-glow").prop("checked",SCMAP.settings.glow),$("#toggle-labels").prop("checked",SCMAP.settings.labels),$("#toggle-label-icons").prop("checked",SCMAP.settings.labelIcons),a=document.createElement("div"),a.id="webgl-container",a.className="noselect webgl-container-noedit",document.body.appendChild(a),e=window.innerWidth||2,f=window.innerHeight||2,camera=new THREE.PerspectiveCamera(45,window.innerWidth/window.innerHeight,10,1600),camera.position.x=cameraDefaults.x,camera.position.y=cameraDefaults.y,camera.position.z=cameraDefaults.z,camera.setViewOffset(e,f,-($("#map_ui").width()/2),0,e,f),controls=new THREE.OrbitControlsFSM(camera,$("#webgl-container")[0]),controls.target=cameraDefaults.target.clone(),controls.restoreOldPosition(),controls.minPolarAngle=0,controls.maxPolarAngle=THREE.Math.degToRad(85),controls.rotateSpeed=$("#gl-info").data("rotateSpeed"),controls.zoomSpeed=$("#gl-info").data("zoomSpeed"),controls.panSpeed=$("#gl-info").data("panSpeed"),controls.noZoom=!1,controls.noPan=!1,controls.mapMode=!0,controls.minDistance=20,controls.maxDistance=800,controls.keyPanSpeed=25,controls.addEventListener("change",render),scene=new THREE.Scene,renderer=new THREE.WebGLRenderer({clearColor:0,clearAlpha:1,antialias:!0}),renderer.setSize(window.innerWidth,window.innerHeight),renderer.autoClear=!1,a.appendChild(renderer.domElement),map=new SCMAP.Map(scene),controls.map=map;var h=[];for(var i in SCMAP.data.systems)h.push(i);var j,k=h.sort(humanSort);for(g=0;g<k.length;g++)i=SCMAP.data.systems[h[g]],j=$('<li><a data-system="'+i.id+'" href="#system='+encodeURI(i.name)+'"><i class="fa-li fa fa-sm fa-crosshairs"></i>'+i.name+"</a></li>"),j.find("a").css("color",i.faction.color.getStyle()),$("#system-list ul").append(j);$("#system-list ul a").bind("click",function(){var a=$(this),b=SCMAP.System.getById(a.data("system"));b.displayInfo(),controls.moveTo(b)});for(var l in SCMAP.Symbols)l=SCMAP.Symbols[l],console.log(l),j=$('<li><i class="fa-li fa '+l.faClass+'"></i>'+l.description+"</li>"),j.css("color",l.color),$("#map_ui ul.legend").append(j);editor=new SCMAP.Editor(map,camera),editor.panSpeed=.6,document.addEventListener("change",render),stats=new Stats,stats.domElement.style.position="absolute",stats.domElement.style.top="0px",stats.domElement.style.right="0px",stats.domElement.style.display="none",stats.domElement.style.zIndex="100",a.appendChild(stats.domElement),window.addEventListener("resize",onWindowResize,!1),b=new THREE.RenderPass(scene,camera),d=new THREE.BloomPass(.75),c=new THREE.ShaderPass(THREE.CopyShader),c.renderToScreen=!0,effectFXAA=new THREE.ShaderPass(THREE.FXAAShader),effectFXAA.uniforms.resolution.value.set(1/(e*dpr),1/(f*dpr)),effectFXAA.enabled=localStorage&&"1"===localStorage["effect.FXAA"]?!0:!1,d.enabled=localStorage&&"1"===localStorage["effect.Bloom"]?!0:!1,$("#toggle-fxaa").prop("checked",effectFXAA.enabled),$("#toggle-bloom").prop("checked",d.enabled),composer=new THREE.EffectComposer(renderer),composer.setSize(e*dpr,f*dpr),composer.addPass(b),composer.addPass(effectFXAA),composer.addPass(d),composer.addPass(c),displayState=buildDisplayModeFSM(localStorage&&localStorage.mode?localStorage.mode:"2d"),$("#3d-mode").prop("checked",localStorage&&"3d"===localStorage.mode),$("#lock-rotation").prop("checked",localStorage&&"1"===localStorage["control.rotationLocked"]),controls.noRotate=localStorage&&"1"===localStorage["control.rotationLocked"],$("#3d-mode").on("change",function(){this.checked?displayState.to3d():displayState.to2d()}),$("#lock-rotation").on("change",function(){controls.noRotate=this.checked,localStorage&&(localStorage["control.rotationLocked"]=this.checked?1:0)}),$("#toggle-fxaa").on("change",function(){effectFXAA.enabled=this.checked,localStorage&&(localStorage["effect.FXAA"]=this.checked?1:0)}),$("#toggle-bloom").on("change",function(){d.enabled=this.checked,localStorage&&(localStorage["effect.Bloom"]=this.checked?1:0)}),$("#toggle-glow").on("change",function(){SCMAP.settings.glow=this.checked,map.updateSystems(),localStorage&&(localStorage["settings.Glow"]=this.checked?1:0)}),$("#toggle-labels").on("change",function(){SCMAP.settings.labels=this.checked,map.updateSystems(),localStorage&&(localStorage["settings.Labels"]=this.checked?1:0)}),$("#toggle-label-icons").on("change",function(){SCMAP.settings.labelIcons=this.checked,map.updateSystems(),localStorage&&(localStorage["settings.LabelIcons"]=this.checked?1:0)}),$("#resetCamera").on("click",function(){controls.cameraTo(cameraDefaults.target,cameraDefaults.theta,cameraDefaults.phi,cameraDefaults.radius)}),$("#centreCamera").on("click",function(){controls.moveTo(cameraDefaults.target)}),$("#northCamera").on("click",function(){controls.rotateTo(0,void 0,void 0)}),$("#topCamera").on("click",function(){controls.rotateTo(0,0,180)}),$("#top2D").on("click",function(){controls.noRotate=!0,$("#lock-rotation").prop("checked",!0),displayState.to2d(),controls.rotateTo(0,0,180)}),$(".quick-button.with-checkbox").on("click",function(){var a=$(this);a.find("input[type=checkbox]").click()}),$("#keyboard-shortcuts").on("click",function(){$(this);$("#keyboard-shortcuts dl").toggle(),$("#keyboard-shortcuts dl").is(":visible")?$("#keyboard-shortcuts > a > i").removeClass("fa-caret-right").addClass("fa-caret-down"):$("#keyboard-shortcuts > a > i").addClass("fa-caret-right").removeClass("fa-caret-down")}),$("#map_ui").on("click","a[data-goto='system']",function(){var a=$(this),b=SCMAP.data.systemsById[a.data("system")];b.displayInfo(),controls.moveTo(b)})}function buildCross(){var a=new THREE.MeshBasicMaterial({wireframe:!0,color:16711680,linewidth:1}),b=new THREE.Object3D,c=new THREE.Geometry;c.vertices.push(new THREE.Vector3(-50,1,0)),c.vertices.push(new THREE.Vector3(50,1,0));var d=new THREE.Line(c,a);b.add(d),c=new THREE.Geometry;var e=new THREE.MeshBasicMaterial({wireframe:!0,color:15790080,linewidth:1});c.vertices.push(new THREE.Vector3(0,1,-50)),c.vertices.push(new THREE.Vector3(0,1,50)),d=new THREE.Line(c,e),b.add(d);var f=new THREE.MeshBasicMaterial({wireframe:!0,color:240,linewidth:1});return c=new THREE.Geometry,c.vertices.push(new THREE.Vector3(0,-50,0)),c.vertices.push(new THREE.Vector3(0,50,0)),d=new THREE.Line(c,f),b.add(d),b}function onWindowResize(){var a=window.innerWidth||2,b=window.innerHeight||2;camera.aspect=a/b,camera.setViewOffset(a,b,-($("#map_ui").width()/2),0,a,b),camera.updateProjectionMatrix(),effectFXAA.uniforms.resolution.value.set(1/(a*dpr),1/(b*dpr)),renderer.setSize(a,b),composer.reset()}function makeSafeForCSS(a){return"string"==typeof a?a.replace(/[^a-zA-Z0-9]/g,function(a){var b=a.charCodeAt(0);return 32==b?"-":b>=65&&90>=b?"_"+a.toLowerCase():b.toString(16).slice(-4)}):void 0}function buildDisplayModeFSM(a){var b,c,d,e;return d={y:"3d"===a?100:.5},b=new TWEEN.Tween(d).to({y:.5},1e3).easing(TWEEN.Easing.Cubic.InOut).onUpdate(function(){map.destroyCurrentRoute();for(var a=0;a<scene.children.length;a++){var b=scene.children[a];"function"==typeof b.scaleY&&b.scaleY(this.y)}}),c=new TWEEN.Tween(d).to({y:100},1e3).easing(TWEEN.Easing.Cubic.InOut).onUpdate(function(){map.destroyCurrentRoute();for(var a=0;a<scene.children.length;a++){var b=scene.children[a];"function"==typeof b.scaleY&&b.scaleY(this.y)}}),e=StateMachine.create({initial:a||"3d",events:[{name:"to2d",from:"3d",to:"2d"},{name:"to3d",from:"2d",to:"3d"}],callbacks:{onenter2d:function(){$("#3d-mode").prop("checked",!1),localStorage&&(localStorage.mode="2d")},onenter3d:function(){$("#3d-mode").prop("checked",!0),localStorage&&(localStorage.mode="3d")},onleave2d:function(){return c.onComplete(function(){e.transition()}),c.start(),StateMachine.ASYNC},onleave3d:function(){return b.onComplete(function(){e.transition()}),b.start(),StateMachine.ASYNC}},error:function(a,b,c,d,e,f){console.log("event "+a+" was naughty : "+f)}})}function animate(){requestAnimationFrame(animate),TWEEN.update(),void 0!==controls&&controls.update(),void 0!==editor&&editor.update(),stats.update(),render()}function render(){map.animateSelector(),renderer.clear(),composer.render()}function hasLocalStorage(){try{return"localStorage"in window&&null!==window.localStorage}catch(a){return!1}}function humanSort(a,b){var c,d,e,f=a.split(/(\d+)/),g=b.split(/(\d+)/);for(c=0;c<Math.max(f.length,g.length);c++)if(f[c]!=g[c])return d=isNaN(parseInt(f[c],10))?f[c]:parseInt(f[c],10),e=isNaN(parseInt(g[c],10))?g[c]:parseInt(g[c],10),void 0===d||void 0===e?f.length-g.length:e>d?-1:1;return 0}if("undefined"==typeof jQuery)throw new Error("starcitizen-webgl-map requires jQuery");if("undefined"==typeof THREE)throw new Error("starcitizen-webgl-map requires THREE.js");var SCMAP=SCMAP||{REVISION:"3"};self.console=self.console||{info:function(){},log:function(){},debug:function(){},warn:function(){},error:function(){}},SCMAP.data={factions:[],crime_levels:[],uee_strategic_values:[],goods:[],map:{},systems:[],systemsById:[]},SCMAP.Symbol={},SCMAP.Symbols={},SCMAP.Symbol.SIZE=24,SCMAP.Symbol.SPACING=9,SCMAP.Symbols.DANGER={code:"",scale:.9,faClass:"fa-warning",description:"Danger, hostile environment!",color:"rgba(255,50,50,0.95)"},SCMAP.Symbols.HANGAR={code:"",scale:1.15,faClass:"fa-home",description:"Hangar location",color:"rgba(255,255,255,0.95)",offset:new THREE.Vector2(0,2)},SCMAP.Symbols.INFO={code:"",scale:1,faClass:"fa-info-circle",description:"Information available",color:"rgba(255, 162, 255, 0.95)"},SCMAP.Symbols.TRADE={code:"",scale:.9,faClass:"fa-exchange",description:"Major trade hub",color:"rgba(255,255,0,0.95)",offset:new THREE.Vector2(-2,-2)},SCMAP.Symbols.BANNED={code:"",scale:1,faClass:"fa-ban",description:"System off-limits",color:"rgba(255, 117, 25, 0.95)"},SCMAP.Symbols.COMMENTS={code:"",scale:.9,faClass:"fa-comment",description:"Your comments",color:"rgba(106, 187, 207, 0.95)"},SCMAP.Symbols.BOOKMARKED={code:"",scale:.9,faClass:"fa-bookmark",description:"Bookmarked",color:"rgba(102, 153, 0, 0.95)"},SCMAP.Settings=function(){this.glow=!0,this.labels=!0,this.labelIcons=!0},SCMAP.settings=new SCMAP.Settings,SCMAP.Editor=function(a,b,c){function d(a){g.enabled!==!1&&void 0!==g.map.selected&&(a.altKey||(a.preventDefault(),0===a.button?n=m.MOVE_XZ:2===a.button&&(n=m.MOVE_Y),h.set(a.clientX,a.clientY),g.domElement.addEventListener("mousemove",e,!1),g.domElement.addEventListener("mouseup",f,!1)))}function e(a){g.enabled!==!1&&void 0!==g.map.selected&&(a.altKey||(a.preventDefault(),(n===m.MOVE_XZ||n===m.MOVE_Y)&&(i.set(a.clientX,a.clientY),j.subVectors(i,h),g.move(j),h.copy(i)),g.update()))}function f(a){g.enabled!==!1&&(a.altKey||(g.domElement.removeEventListener("mousemove",e,!1),g.domElement.removeEventListener("mouseup",f,!1),n=m.NONE))}this.map=a,this.camera=b,this.domElement=void 0!==c?c:document,this.enabled=!1;var g=this,h=new THREE.Vector2,i=new THREE.Vector2,j=new THREE.Vector2,k=new THREE.Vector3,l=new THREE.Vector3,m={NONE:-1,MOVE_XZ:0,MOVE_Y:1},n=m.NONE,o={type:"change"};this.moveLeft=function(a){var b=window.controls.target.clone().setY(this.camera.position.y),c=this.camera.position.clone().sub(b).negate().setLength(a),d=new THREE.Vector3(0,1,0);c.applyAxisAngle(d,THREE.Math.degToRad(90)),k.add(c)},this.moveUp=function(a){var b=new THREE.Vector3,c=this.map.selected.object.matrix.elements;b.set(c[4],c[5],c[6]),b.multiplyScalar(a),k.add(b)},this.moveBack=function(a){var b=window.controls.target.clone().setY(this.camera.position.y),c=this.camera.position.clone().sub(b).negate().setLength(a);k.add(c)},this.move=function(a){var b=g.domElement===document?g.domElement.body:g.domElement,c=g.map.selected.object.position.clone().sub(g.camera.position),d=c.length();d*=Math.tan(g.camera.fov/2*Math.PI/180),n===m.MOVE_Y?g.moveUp(2*a.y*d/b.clientHeight):(g.moveLeft(2*a.x*d/b.clientHeight),g.moveBack(2*a.y*d/b.clientHeight))},this.update=function(){if(g.enabled!==!1&&void 0!==this.map.selected){this.map.selected.object.position.sub(k),void 0!==this.map.selected.object.system.labelObject&&this.map.selected.object.system.labelObject.position.sub(k);for(var a=0;a<this.map.selected.object.system.routeObjects.length;a++){var b=this.map.selected.object.system.routeObjects[a];b.geometry.verticesNeedUpdate=!0}k.set(0,0,0),l.distanceTo(this.map.selected.object.position)>0&&(this.dispatchEvent(o),l.copy(this.map.selected.object.position))}},this.domElement.addEventListener("contextmenu",function(a){a.preventDefault()},!1),this.domElement.addEventListener("mousedown",d,!1)},SCMAP.Editor.prototype=Object.create(THREE.EventDispatcher.prototype),SCMAP.SelectedSystemGeometry=SCMAP.SelectedSystemGeometry||_build_selected_system_object(),SCMAP.JumpPoint=function(a){this.name="string"==typeof a.name&&a.name.length>1?a.name:void 0,this.source=a.source instanceof SCMAP.System?a.source:void 0,this.destination=a.destination instanceof SCMAP.System?a.destination:void 0,this.is_valid=!1,this.drawn=!1,this.typeId="number"==typeof a.typeId?a.typeId:4,this.entryAU=new THREE.Vector3("number"==typeof a.entryAU[0]?a.entryAU[0]:0,"number"==typeof a.entryAU[1]?a.entryAU[1]:0,"number"==typeof a.entryAU[2]?a.entryAU[2]:0),void 0===this.source||void 0===this.destination||this.source===this.destination?console.error("Invalid route created"):(this.is_valid=!0,(void 0===this.name||""===this.name)&&(this.name="["+this.source.name+" to "+this.destination.name+"]"))},SCMAP.JumpPoint.prototype={constructor:SCMAP.JumpPoint,length:function(){return this.is_valid?this.source.position.distanceTo(this.destination.position):void 0},buildSceneObject:function(){var a,b,c;if(!this.drawn){for(a=0;a<this.destination.jumpPoints.length;a++)if(b=this.destination.jumpPoints[a],b.destination==this.source&&b.drawn)return;for(c=new THREE.Geometry,c.colors.push(this.source.faction.lineColor),c.vertices.push(this.source.sceneObject.position),c.colors.push(this.destination.faction.lineColor),c.vertices.push(this.destination.sceneObject.position),this.setDrawn(),a=0;a<this.destination.jumpPoints.length;a++)b=this.destination.jumpPoints[a],b.destination==this.source&&b.setDrawn();return c.computeLineDistances(),new THREE.Line(c,this.getMaterial(),THREE.LinePieces)}},getMaterial:function(){return 2===this.typeId?SCMAP.JumpPoint.Material.Undiscovered:4===this.typeId?SCMAP.JumpPoint.Material.Possible:SCMAP.JumpPoint.Material.Regular},setDrawn:function(){this.drawn=!0}},SCMAP.JumpPoint.Material={},SCMAP.JumpPoint.Material.Regular=new THREE.LineBasicMaterial({color:16777215,linewidth:2,vertexColors:!0}),SCMAP.JumpPoint.Material.Undiscovered=new THREE.LineDashedMaterial({color:16777215,dashSize:.75,gapSize:.75,linewidth:2,vertexColors:!0}),SCMAP.JumpPoint.Material.Possible=new THREE.LineDashedMaterial({color:16777215,dashSize:2,gapSize:2,linewidth:2,vertexColors:!0}),SCMAP.Faction=function(a){this.id=void 0,this.name="Unclaimed",this.shortName="NONE",this.isRealFaction=!1,this.color=new THREE.Color(16777215),this.planeColor=new THREE.Color(16711680),this.lineColor=new THREE.Color(16776960),this.setValues(a),this._claimed={systems:{}},this._darkMaterial=void 0},SCMAP.Faction.prototype={constructor:SCMAP.Faction,claim:function(a){return!a instanceof SCMAP.System?(new Error("A faction can only claim ownership over a system"),void 0):(this._claimed.systems[a.uuid]=!0,this)},claimed:function(a){return!a instanceof SCMAP.System?(new Error("A faction can only test ownership over a system"),void 0):this._claimed.systems[a.uuid]},material:function(){return"undefined"==typeof this._darkMaterial&&(this._darkMaterial=new THREE.MeshBasicMaterial({color:this.planeColor,vertexColors:!0})),this._darkMaterial},getValue:function(a){if(void 0!==a){var b=this[a];return b}},setValues:function(a){if(void 0!==a)for(var b in a){var c=a[b];if(void 0!==c){if(b in this){var d=this[b];d instanceof THREE.Color?(c instanceof THREE.Color?this[b]=c:(c=c.replace("0x","#"),this[b]=new THREE.Color(c)),"color"===b&&(this.planeColor=this[b].clone().offsetHSL(0,.5,0).multiplyScalar(.2),this.lineColor=this[b].clone().offsetHSL(0,.05,-.05))):this[b]=c}}else console.log('SCMAP.Faction: "'+b+'" parameter is undefined for "'+this.name+'"')}}},SCMAP.Faction.preprocessFactions=function(){var a,b;SCMAP.data.factionsByName={};for(a in SCMAP.data.factions)b=SCMAP.data.factions[a],b instanceof SCMAP.Faction?(SCMAP.data.factionsByName[b.name]=b,SCMAP.data.factionsByName[b.shortName]=b):(b=new SCMAP.Faction({id:a,name:b.name,shortName:b.short_name,color:b.color,isRealFaction:b.is_real_faction}),SCMAP.data.factions[a]=b,SCMAP.data.factionsByName[b.name]=b,SCMAP.data.factionsByName[b.shortName]=b)},SCMAP.Faction.getById=function(a){var b=SCMAP.data.factions[a];return b instanceof SCMAP.Faction||(b=SCMAP.data.factionsByName.Unclaimed),b},SCMAP.Faction.getByName=function(a){var b=SCMAP.data.factionsByName[a];return b instanceof SCMAP.Faction||(b=SCMAP.data.factionsByName.Unclaimed),b},SCMAP.Goods=function(a){this.id=void 0,this.name="Unknown",this.isBlackMarket=!1,this.setValues(a),this._trade={importing:[],exporting:[],blackMarket:[]}},SCMAP.Goods.prototype={constructor:SCMAP.Goods,getValue:function(a){if(void 0!==a){var b=this[a];return b}},setValues:function(a){if(void 0!==a)for(var b in a){var c=a[b];if(void 0!==c){if(b in this){{this[b]}this[b]=c}}else console.log('SCMAP.Goods: "'+b+'" parameter is undefined for "'+this.name+'"')}}},SCMAP.Goods.preprocessGoods=function(){var a,b;SCMAP.data.goodsByName={};for(a in SCMAP.data.goods)b=SCMAP.data.goods[a],b instanceof SCMAP.Goods?SCMAP.data.goodsByName[b.name]=b:(b=new SCMAP.Goods({id:a,name:b.name,isBlackMarket:b.black_market}),SCMAP.data.goods[a]=b,SCMAP.data.goodsByName[b.name]=b)},SCMAP.Goods.getById=function(a){return SCMAP.data.goods[a]},SCMAP.Goods.getByName=function(a){return SCMAP.data.goodsByName[a]},SCMAP.System=function(a){this.id=void 0,this.uuid=void 0,this.name="",this.nickname="",this.position=new THREE.Vector3,this.faction=new SCMAP.Faction,this.size="medium",this.jumpPoints=[],this.starColor=new THREE.Color(16777215),this.source=void 0,this.planets=0,this.planetaryRotation=[],this.import=[],this.export=[],this.crimeStatus="",this.blackMarket=[],this.ueeStrategicValue=void 0,this.blob=[],this.setValues(a),this._routeObjects=[],this.scale=1,this.binary=!1},SCMAP.System.prototype={constructor:SCMAP.System,buildSceneObject:function(){var a,b,c,d;return this.sceneObject=new THREE.Object3D,a=new THREE.Mesh(SCMAP.System.MESH,this.starMaterial()),a.scale.set(this.scale,this.scale,this.scale),this.sceneObject.add(a),c=new THREE.Sprite(this.glowMaterial()),c.scale.set(SCMAP.System.GLOW_SCALE*this.scale,SCMAP.System.GLOW_SCALE*this.scale,1),c.isGlow=!0,c.sortParticles=!0,c.visible=SCMAP.settings.glow,this.sceneObject.add(c),b=new THREE.Sprite(this.labelSprite(SCMAP.settings.labelIcons)),b.position.set(0,3.5,0),b.position.set(0,3*this.scale,0),b.scale.set(SCMAP.System.LABEL_SCALE*b.material.map.image.width,SCMAP.System.LABEL_SCALE*b.material.map.image.height,1),b.isLabel=!0,b.sortParticles=!0,b.visible=SCMAP.settings.labels,this.sceneObject.add(b),d=this.position.clone(),localStorage&&"2d"===localStorage.mode&&d.setY(.005*d.y),this.sceneObject.position=d,this.sceneObject.system=this,this.sceneObject.scaleY=this.scaleY,this.sceneObject},updateSceneObject:function(){for(var a=0;a<this.sceneObject.children.length;a++){var b=this.sceneObject.children[a];b.isLabel?(this.updateLabelSprite(b.material,SCMAP.settings.labelIcons),b.visible=SCMAP.settings.labels):b.isGlow&&(b.visible=SCMAP.settings.glow)}},starMaterial:function(){return SCMAP.System.STAR_MATERIAL_WHITE},glowMaterial:function(){var a=this.starColor;return a.equals(SCMAP.Color.BLACK)&&a.copy(SCMAP.Color.UNSET),new THREE.SpriteMaterial({map:SCMAP.System.GLOW_MAP,blending:THREE.AdditiveBlending,transparent:!1,useScreenCoordinates:!1,color:a})},labelSprite:function(a){var b,c,d;return b=this.drawSystemText(a),c=new THREE.Texture(b),c.needsUpdate=!0,d=new THREE.SpriteMaterial({map:c,useScreenCoordinates:!1,blending:THREE.CustomBlending})},updateLabelSprite:function(a,b){var c,d;c=this.drawSystemText(b),d=new THREE.Texture(c),d.needsUpdate=!0,a.map=d},drawSystemText:function(a){var b,c,d,e,f,g=this.name;for(b=document.createElement("canvas"),b.width=256,b.height=256,c=b.getContext("2d"),c.font="36pt Electrolize, Calibri, sans-serif",c.textAlign="center",c.strokeStyle="rgba(0,0,0,0.95)",c.lineWidth=5,d=Math.ceil(c.measureText(g).width+1);d>b.width;)b.width*=2,b.height*=2;if(e=b.width/2,f=b.height/2,c.font="36pt Electrolize, Calibri, sans-serif",c.strokeStyle="rgba(0,0,0,0.95)",c.textAlign="center",c.lineWidth=5,c.strokeText(g,e,f),c.fillStyle=this.faction.color.getStyle(),c.fillText(g,e,f),a){var h=[];"Vanduul"===this.faction.name&&h.push(SCMAP.Symbols.DANGER),"Sol"===this.name&&h.push(SCMAP.Symbols.HANGAR),this.blob.length&&h.push(SCMAP.Symbols.INFO),Math.random()>.8&&h.push(SCMAP.Symbols.TRADE),this._drawSymbols(c,e,f-50,h)}return b},_drawSymbols:function(a,b,c,d){var e,f,g=SCMAP.Symbol.SIZE*d.length+SCMAP.Symbol.SPACING*(d.length-1);for(b-=g/2,e=0;e<d.length;e++)f=d[e],a.font=(SCMAP.Symbol.SIZE*f.scale).toFixed(1)+"pt FontAwesome",f.offset&&(b+=f.offset.x,c+=f.offset.y),a.strokeStyle="rgba(0,0,0,0.95)",a.textAlign="center",a.lineWidth=5,a.strokeText(f.code,b+SCMAP.Symbol.SIZE/2,c),a.fillStyle=f.color,a.fillText(f.code,b+SCMAP.Symbol.SIZE/2,c),b+=SCMAP.Symbol.SIZE+SCMAP.Symbol.SPACING},createInfoLink:function(){var a=this,b=$("<a></a>");return"undefined"!=typeof a.faction&&"undefined"!=typeof a.faction&&b.css("color",a.faction.color.getStyle()),b.attr("data-goto","system"),b.attr("data-system",a.id),b.attr("href","#system="+encodeURI(a.name)),b.attr("title","Show information on "+a.name),b.html('<i class="fa fa-crosshairs"></i>&nbsp;'+a.name),b},displayInfo:function(a){"undefined"==typeof a&&(a=this);var b="(No information)",c="&mdash;",d="&mdash;",e="&mdash;",f="Unknown",g="Unknown",h=$('<div class="sc_system_info '+makeSafeForCSS(a.name)+'"></div>'),i=window.map.indexOfCurrentRoute(a);if($("#systemname").attr("class",makeSafeForCSS(a.faction.name)).css("color",a.faction.color.getStyle()).text("System: "+a.name),"string"==typeof a.nickname&&a.nickname.length&&h.append('<h2 class="nickname">'+a.nickname+"</h2>"),"number"==typeof i){var j=window.map.currentRoute(),k=[];if(i>0){var l=j[i-1].system.createInfoLink();l.attr("title","Previous jump to "+j[i-1].system.name+" ("+j[i-1].system.faction.name+" territory)"),l.empty().append('<i class="left fa fa-fw fa-arrow-left"></i>'),k.push(l)}else k.push($('<i class="left fa fa-fw"></i>'));if(i<j.length-1){var m=j[i+1].system.createInfoLink();m.attr("title","Next jump to "+j[i+1].system.name+" ("+j[i+1].system.faction.name+" territory)"),m.empty().append('<i class="right fa fa-fw fa-arrow-right"></i>'),k.push(m)}else k.push($('<i class="right fa fa-fw"></i>'));k.push(a.name),$("#systemname").empty().attr("class",makeSafeForCSS(a.faction.name)).append(k)}if(a.planetaryRotation.length&&(b=a.planetaryRotation.join(", ")),a.import.length&&(c=$.map(a.import,function(a){return SCMAP.data.goods[a].name}).join(", ")),a.export.length&&(d=$.map(a.export,function(a){return SCMAP.data.goods[a].name}).join(", ")),a.blackMarket.length&&(e=$.map(a.blackMarket,function(a){return SCMAP.data.goods[a].name}).join(", ")),"object"==typeof a.crimeStatus&&(g=a.crimeStatus.name),"object"==typeof a.ueeStrategicValue&&(f=a.ueeStrategicValue.color),h.append('<dl><dt class="faction">Faction</dt><dd class="faction">'+a.faction.name+'</dd><dt class="rotation">Planetary rotation</dt><dd class="rotation">'+b+'</dd><dt class="import">Import</dt><dd class="import">'+c+'</dd><dt class="export">Export</dt><dd class="export">'+d+'</dd><dt class="blackMarket">Black market</dt><dd class="crime">'+e+'</dd><dt class="crime_'+g.toLowerCase()+'">Crime status</dt><dd class="crime">'+g+'</dd><dt class="strategic_value_'+f.toLowerCase()+'">UEE strategic value</dt><dd class="strategic">'+f+"</dd></dl>"),"Unclaimed"!==a.faction.name&&h.find("dd.faction").css("color",a.faction.color.getStyle()),a.blob.length){var n=$(markdown.toHTML(a.blob));n.find("p").prepend('<i class="fa fa-2x fa-quote-left"></i>'),h.append('<div id="systemInfo">',n,"</div>")}a.source&&h.append('<p><a class="system-source-url" href="'+a.source+'" target="_blank">(source)</a></p>'),h.append('<div id="destinations">'),$("#systemblurb").empty(),$("#systemblurb").append(h),$("#map_ui").tabs("option","active",2),$("#map_ui").data("jsp").reinitialise()},scaleY:function(a){var b=this.system.position.y*(a/100);this.system.sceneObject.translateY(b-this.system.sceneObject.position.y);for(var c=0;c<this.system._routeObjects.length;c++)this.system._routeObjects[c].geometry.verticesNeedUpdate=!0},moveTo:function(a){this.sceneObject.position.copy(a);for(var b=0;b<this._routeObjects.length;b++)this._routeObjects[b].geometry.verticesNeedUpdate=!0},translateVector:function(a){this.sceneObject.add(a);for(var b=0;b<this._routeObjects.length;b++)this._routeObjects[b].geometry.verticesNeedUpdate=!0},getValue:function(a){if(void 0!==a){var b=this[a];return b}},setValues:function(a){var b,c,d;if(void 0!==a)for(b in a)if(d=a[b],void 0!==d){if(b in this){if(c=this[b],"size"==b){switch(d){case"dwarf":this.scale=.6;break;case"medium":this.scale=1;break;case"large":this.scale=1.25;break;case"giant":this.scale=1.6;break;case"binary":this.scale=1.6,this.binary=!0}this[b]=d}c instanceof THREE.Color?d instanceof THREE.Color?this[b]=d:(d=d.replace("0x","#"),this[b]=new THREE.Color(d)):c instanceof SCMAP.Faction?this[b]=d.claim(this):c instanceof THREE.Vector3&&d instanceof THREE.Vector3?c.copy(d):c instanceof THREE.Vector3?d instanceof THREE.Vector3?c.copy(d):d instanceof Array&&c.fromArray(d):this[b]=d}}else console.log('SCMAP.System: "'+b+'" parameter is undefined for "'+this.name+'"')}},SCMAP.System.preprocessSystems=function(){var a,b,c,d,e;SCMAP.data.systemsById={},SCMAP.System.List=[];for(b in SCMAP.data.systems)d=SCMAP.data.systems[b],d instanceof SCMAP.System?(SCMAP.data.systemsById[c.id]=d,SCMAP.data.systemsById[c.uuid]=d):(e=SCMAP.Faction.getById(d.faction_id),c=new SCMAP.System({id:d.system_id,uuid:d.uuid,name:b,position:d.coords,scale:d.scale||1,starColor:d.color,faction:e}),c.setValues({nickname:d.nickname,size:d.size,source:d.source,crimeStatus:SCMAP.data.crime_levels[d.crime_level_id],ueeStrategicValue:SCMAP.data.uee_strategic_values[""+d.uee_strategic_value_id],"import":d.import,"export":d.export,blackMarket:d.black_market,blob:d.blob,planets:0,planetaryRotation:[],jumpPoints:d.jumppoints}),SCMAP.data.systems[c.name]=c,SCMAP.data.systemsById[c.id]=c,SCMAP.data.systemsById[c.uuid]=c);for(b in SCMAP.data.systems)for(c=SCMAP.System.getByName(b),SCMAP.System.List.push(c),a=0;a<c.jumpPoints.length;a++)jumpPoint=c.jumpPoints[a],c.jumpPoints[a]=new SCMAP.JumpPoint({source:c,destination:SCMAP.System.getById(jumpPoint.destination),name:jumpPoint.name,typeId:jumpPoint.type_id,entryAU:jumpPoint.coords_au})},SCMAP.System.List=[],SCMAP.System.getByName=function(a){return SCMAP.data.systems[a]},SCMAP.System.getById=function(a){return SCMAP.data.systemsById[a]},SCMAP.Color={},SCMAP.Color.BLACK=new THREE.Color("black"),SCMAP.Color.UNSET=new THREE.Color(8429772),SCMAP.System.COLORS={RED:16736352,BLUE:6316287,WHITE:16777215,YELLOW:16777056,ORANGE:15790208,UNKNOWN:16777215},SCMAP.System.LABEL_SCALE=.06,SCMAP.System.GLOW_SCALE=6,SCMAP.System.MESH=new THREE.SphereGeometry(1,12,12),SCMAP.System.STAR_MATERIAL_RED=new THREE.MeshBasicMaterial({color:SCMAP.System.COLORS.RED,name:"STAR_MATERIAL_RED"}),SCMAP.System.STAR_MATERIAL_BLUE=new THREE.MeshBasicMaterial({color:SCMAP.System.COLORS.BLUE,name:"STAR_MATERIAL_BLUE"}),SCMAP.System.STAR_MATERIAL_WHITE=new THREE.MeshBasicMaterial({color:SCMAP.System.COLORS.WHITE,name:"STAR_MATERIAL_WHITE"}),SCMAP.System.STAR_MATERIAL_YELLOW=new THREE.MeshBasicMaterial({color:SCMAP.System.COLORS.YELLOW,name:"STAR_MATERIAL_YELLOW"}),SCMAP.System.STAR_MATERIAL_ORANGE=new THREE.MeshBasicMaterial({color:SCMAP.System.COLORS.ORANGE,name:"STAR_MATERIAL_ORANGE"}),SCMAP.System.STAR_MATERIAL_UNKNOWN=new THREE.MeshBasicMaterial({color:SCMAP.System.COLORS.UNKNOWN,name:"STAR_MATERIAL_UNKNOWN"}),SCMAP.System.GLOW_MAP=new THREE.ImageUtils.loadTexture($("#gl-info").data("glow-image")),SCMAP.System.GLOW_MATERIAL_RED=new THREE.SpriteMaterial({map:SCMAP.System.GLOW_MAP,blending:THREE.AdditiveBlending,color:SCMAP.System.COLORS.RED}),SCMAP.System.GLOW_MATERIAL_BLUE=new THREE.SpriteMaterial({map:SCMAP.System.GLOW_MAP,blending:THREE.AdditiveBlending,color:SCMAP.System.COLORS.BLUE}),SCMAP.System.GLOW_MATERIAL_WHITE=new THREE.SpriteMaterial({map:SCMAP.System.GLOW_MAP,blending:THREE.AdditiveBlending,color:SCMAP.System.COLORS.WHITE}),SCMAP.System.GLOW_MATERIAL_YELLOW=new THREE.SpriteMaterial({map:SCMAP.System.GLOW_MAP,blending:THREE.AdditiveBlending,color:SCMAP.System.COLORS.YELLOW}),SCMAP.System.GLOW_MATERIAL_ORANGE=new THREE.SpriteMaterial({map:SCMAP.System.GLOW_MAP,blending:THREE.AdditiveBlending,color:SCMAP.System.COLORS.ORANGE}),SCMAP.System.GLOW_MATERIAL_UNKNOWN=new THREE.SpriteMaterial({map:SCMAP.System.GLOW_MAP,blending:THREE.AdditiveBlending,color:SCMAP.System.COLORS.UNKNOWN}),SCMAP.Dijkstra=function(a){if("object"!=typeof a||!Array.isArray(a))return console.error("No array specified to SCMAP.Dijkstra constructor!"),void 0;this._nodes=[],this._mapping={};for(var b=a.length;b--;)this._nodes[b]={system:a[b],distance:Number.POSITIVE_INFINITY,previous:null},this._mapping[a[b].id]=this._nodes[b];this._result={}},SCMAP.Dijkstra.prototype={constructor:SCMAP.Dijkstra,buildGraph:function(a){var b,c,d,e,f,g,h=new Date;if(a instanceof SCMAP.System){if(this._result.source instanceof SCMAP.System&&this._result.source===a)return console.log("Reusing generated graph starting at",a.name),void 0;for(this.destroyGraph(),this._result.source=a,console.log("Building graph, starting at",a.name),c=0;c<this._nodes.length;c++)this._nodes[c].distance=Number.POSITIVE_INFINITY,this._nodes[c].previous=null;for(e=this._mapping[a.id],e.distance=0,e.previous=null,b=SCMAP.Dijkstra.quickSort(this._nodes);b.length>=1&&(e=b[0],b.splice(0,1),!isInfinite(e.distance));)for(c=0;c<e.system.jumpPoints.length;c++)f=e.system.jumpPoints[c],d=e.distance+f.length(),d<this._mapping[f.destination.id].distance&&(this._mapping[f.destination.id].distance=d,this._mapping[f.destination.id].previous=e,b=SCMAP.Dijkstra.quickSort(b));this._result.nodes=b,g=new Date,console.log("Graph building took "+(g.getTime()-h.getTime())+" msec")}},destroyGraph:function(){this._result={}},routeArray:function(a){if(!(a instanceof SCMAP.System))return console.error("No or invalid destination specified."),void 0;if(this._result.nodes.length>0){for(var b=[],c=this._mapping[a.id],d={};null!==c;)d[c.system.name]=!0,b.push(c),c=c.previous;return b.reverse(),b}},constructRouteObject:function(a,b,c){var d,e,f;if(a instanceof SCMAP.System&&b instanceof SCMAP.System){if("function"!=typeof c)return console.error("Callback not given or not a function"),void 0;if(this.buildGraph(a),d=this.routeArray(b),"object"==typeof d&&Array.isArray(d)){for(f=new THREE.Object3D,e=0;e<d.length-1;e++)f.add(c(d[e+0].system,d[e+1].system));return f}}}},SCMAP.Dijkstra.quickSort=function(a){for(var b=[],c=a.length;c--;)b[c]=a[c];if(b.length<=1)return b;var d=[],e=[],f=Math.ceil(b.length/2)-1;for(f=b.splice(f,1)[0],c=0;c<b.length;c++)b[c].distance<=f.distance?d.push(b[c]):e.push(b[c]);var g=SCMAP.Dijkstra.quickSort(d),h=SCMAP.Dijkstra.quickSort(e);return g.push(f),g.concat(h)},SCMAP.Map=function(a){this.name="Star Citizen 'Verse",this.scene=a,this.goods={},this._selected=void 0,this._destination=void 0,this.group=void 0,this._interactables=[],this.referencePlane=void 0,this._selectorObject=this.createSelectorObject(),this.scene.add(this._selectorObject),this.canEdit=!1,$("#map_ui li.editor").hide(),this.populateScene(),this._graph=new SCMAP.Dijkstra(SCMAP.System.List),this._routeObject=void 0
},SCMAP.Map.prototype={constructor:SCMAP.Map,createSelectorObject:function(){var a=new THREE.Mesh(SCMAP.SelectedSystemGeometry,new THREE.MeshBasicMaterial({color:13421772,transparent:!0,blending:THREE.AdditiveBlending}));return a.scale.set(4.2,4.2,4.2),a.visible=!1,a.systemPosition=new THREE.Vector3(0,0,0),a.scaleY=function(a){var b=this.systemPosition.y*(a/100);this.translateY(b-this.position.y)},a},updateSelectorObject:function(a){a instanceof SCMAP.System?(this._selectorObject.visible=!0,this._selectorObject.systemPosition.copy(a.position),this.moveSelectorTo(a),this._selected=a):(this._selectorObject.visible=!1,this._selected=void 0)},system:function(a){return SCMAP.System.getByName(a)},selected:function(){return this._selected},interactables:function(){return this._interactables},deselect:function(){this._selectorObject.visible=!1,this._selected=void 0},animateSelector:function(){this._selectorObject.visible&&(this._selectorObject.rotation.y=200*THREE.Math.degToRad(25e-5*Date.now()))},updateSystems:function(){for(var a=0;a<SCMAP.System.List.length;a++)SCMAP.System.List[a].updateSceneObject(this.scene)},moveSelectorTo:function(a){var b,c,d,e,f=this;if(!(f._selectorObject.visible&&f._selected instanceof SCMAP.System))return f._selectorObject.systemPosition.copy(a.position),f._selectorObject.position.copy(a.sceneObject.position),f._selectorObject.visible=!0,f._selected=a,void 0;c=a.sceneObject.position.clone();var g=new SCMAP.Dijkstra(SCMAP.System.List);g.buildGraph(f._selected);var h=g.routeArray(a);if(h.length<=1)return f._selectorObject.systemPosition.copy(a.position),f._selectorObject.position.copy(a.sceneObject.position),f._selectorObject.visible=!0,f._selected=a,void 0;f._selectorObject.position.copy(f._selectorObject.position),d={x:f._selectorObject.position.x,y:f._selectorObject.position.y,z:f._selectorObject.position.z};var j=[];for(i=0;i<h.length-1;i++)e=h[i+1].system,b=new TWEEN.Tween(d).to({x:e.sceneObject.position.x,y:e.sceneObject.position.y,z:e.sceneObject.position.z},800/(h.length-1)).easing(TWEEN.Easing.Linear.None).onUpdate(function(){f._selectorObject.position.setX(this.x),f._selectorObject.position.setY(this.y),f._selectorObject.position.setZ(this.z)}),0==i&&(2==h.length?b.easing(TWEEN.Easing.Cubic.InOut):b.easing(TWEEN.Easing.Cubic.In)),i>0&&j[i-1].chain(b),i==h.length-2&&(b.easing(TWEEN.Easing.Cubic.Out),b.onComplete(function(){f._selectorObject.systemPosition.copy(e.position),f._selectorObject.position.copy(e.sceneObject.position),f._selected=a})),j.push(b);j[0].start()},handleSelection:function(a,b){if("object"==typeof b){var c=a.shiftKey||a.ctrlKey?!0:!1;"mousedown"===a.type?c?this._destination=b.object.parent.system:this.moveSelectorTo(b.object.parent.system):"mouseup"===a.type&&(window.editor.enabled||(c?this.updateRoute(b.object.parent.system):this._selected instanceof SCMAP.System&&b.object.parent.system instanceof SCMAP.System&&b.object.parent.system===this._selected&&(this.updateSelectorObject(b.object.parent.system),b.object.parent.system.displayInfo())))}},currentRoute:function(){return this._destination instanceof SCMAP.System?this._graph.routeArray(this._destination):[]},indexOfCurrentRoute:function(a){if(!(!a instanceof SCMAP.System)){var b,c=this.currentRoute();if(c.length)for(i=0;i<c.length;i++)if(c[i].system===a){b=i;break}return b}},destroyCurrentRoute:function(){this._routeObject&&scene.remove(this._routeObject)},updateRoute:function(a){var b,c,d,e,f,g=this;if(d=new THREE.LineBasicMaterial({color:16711935,linewidth:2.5}),this.destroyCurrentRoute(),this._routeObject=g._graph.constructRouteObject(g._selected,a,function(a,b){var c=g.createRouteMesh(a,b),e=new THREE.Line(c,d);return e.position=a.sceneObject.position.clone(),e.lookAt(b.sceneObject.position),e}),this._routeObject){for(this.scene.add(this._routeObject),this._destination=a,c=this._graph.routeArray(a),$("#routelist").empty(),$("#routelist").append("<p>The shortest route from "+c[0].system.createInfoLink().outerHtml()+" to "+c[c.length-1].system.createInfoLink().outerHtml()+" along <strong>"+(c.length-1)+"</strong> jump points:</p>").append('<ol class="routelist"></ol>'),b=0;b<c.length;b++)e=c[b+0].system,f=$("<li></li>").append(e.createInfoLink()),$("#routelist ol").append(f);$("#map_ui").tabs("option","active",3)}},createRouteMesh:function(a,b){var c,d,e,f=2*Math.PI/16,g=.5,h=.5,i=new THREE.Geometry,j=a.sceneObject.position.distanceTo(b.sceneObject.position),k=0;for(c=0;j>k;c+=f)d=h*Math.cos(c),e=0-h*Math.sin(c),i.vertices.push(new THREE.Vector3(d,e,k)),k+=g;return i},populateScene:function(){var a,b,c,d,e,f,g,h=0;e=f=new Date,SCMAP.Faction.preprocessFactions(),SCMAP.Goods.preprocessGoods(),SCMAP.System.preprocessSystems();for(b in SCMAP.data.systems)a=SCMAP.System.getByName(b),sceneObject=a.buildSceneObject(),this.scene.add(sceneObject),this._interactables.push(sceneObject.children[0]),h++;for(b in SCMAP.data.systems)for(a=SCMAP.System.getByName(b),g=0;g<a.jumpPoints.length;g++)c=a.jumpPoints[g],d=c.buildSceneObject(),d instanceof THREE.Object3D&&(a._routeObjects.push(d),this.scene.add(d));e=new Date,console.log("Populating the scene (without ref plane) took "+(e.getTime()-f.getTime())+" msec"),$("#debug-systems").html(h+" systems loaded"),this.buildReferencePlane()},closestPOI:function(a){var b,c,d,e,f,g,h=1/0;for(e in SCMAP.data.systems)c=SCMAP.System.getByName(e),f=a.x-c.position.x,g=a.z-c.position.z,d=Math.sqrt(f*f+g*g),h>d&&(h=d,b=c);return[h,b]},closestFromArray:function(a,b){for(var c,d,e,f,g,h=1/0,i=0;i<b.length;i++)d=b[i],f=a.x-d.position.x,g=a.z-d.position.z,e=Math.sqrt(f*f+g*g),h>e&&(h=e,c=d);return[h,c]},withinApproxDistance:function(a,b){var c=[];for(var d in SCMAP.data.systems)system=SCMAP.System.getByName(d),system.position.x<a.x-b||system.position.x>a.x+b||system.position.z<a.z-b||system.position.z>a.z+b||c.push(system);return c},furthestPOI:function(a){var b,c,d,e,f,g,h=0;for(e in SCMAP.data.systems)c=SCMAP.System.getByName[e],f=a.x-c.position.x,g=a.z-c.position.z,d=Math.sqrt(f*f+g*g),d>h&&(h=d,b=c);return[h,b]},referencePlaneTerritoryColor:function(){if(!(!this.referencePlane instanceof THREE.Object3D))for(var a=this.referencePlane.geometry,b=55,c=0;c<a.vertices.length;c++){var d=a.vertices[c],e=this.closestPOI(d),f=e[0],g=e[1];f>b&&(f=b);var h=(b-f)/b,i=g.faction.planeColor.clone();i.multiplyScalar(.8*h),a.colors[c]=i}},referencePlaneSolidColor:function(a){var b,c,d=this.referencePlane.geometry;if(!(!this.referencePlane instanceof THREE.Object3D))for(b=0;b<d.vertices.length;b++)c=d.vertices[b],d.colors[b]=a},pointAtPlane:function(a,b,c){return new THREE.Vector3(b*Math.cos(a),c,-b*Math.sin(a))},referencePlaneTerritoryColourMesh:function(a,b,c,d,e){var f,g;return f=new THREE.Geometry,f.vertices.push(this.pointAtPlane(b,d,-.04)),f.vertices.push(this.pointAtPlane(c,d,-.04)),f.vertices.push(this.pointAtPlane(c,e,-.04)),f.vertices.push(this.pointAtPlane(b,e,-.04)),f.faces.push(new THREE.Face3(2,1,0)),f.faces.push(new THREE.Face3(3,2,0)),g=new THREE.Mesh(f,a)},buildReferencePlane:function(){var a,b,c,d,e=10,f=0,g=0,h=0,i=0;c=d=new Date;for(b in SCMAP.data.systems)a=SCMAP.System.getByName(b).position,a.x<f&&(f=a.x),a.x>h&&(h=a.x),a.z<g&&(g=a.z),a.z>i&&(i=a.z);f-=4*e,g-=4*e,h+=4*e,i+=4*e,f=Math.floor(f/e)*e,g=Math.floor(g/e)*e,h=Math.floor(h/e)*e,i=Math.floor(i/e)*e;for(var j,k=Math.floor(f/e),l=Math.floor(g/e),m=Math.floor(h/e),n=Math.floor(i/e),o=new THREE.Geometry,p=new THREE.Color(0),q=new THREE.LineBasicMaterial({linewidth:1.5,wireframe:!0,vertexColors:THREE.VertexColors}),r=l;n>=r;r++)for(var s=r*e,t=(r+1)*e,u=k;m>=u;u++){var v=u*e,w=(u+1)*e,x=new THREE.Vector3(v,0,s),y=new THREE.Vector3(w,0,s),z=new THREE.Vector3(w,0,t);o.vertices.push(x),o.vertices.push(y),o.vertices.push(y),o.vertices.push(z);var A=this.withinApproxDistance(y,55),B=this.closestFromArray(x,A);B[0]<=45&&B[1]?(j=B[1].faction.planeColor.clone(),B[0]>=40?j.multiplyScalar(.5):B[0]>=30&&j.multiplyScalar(.8)):j=p,o.colors.push(j),B=this.closestFromArray(y,A),B[0]<=45&&B[1]?(j=B[1].faction.planeColor.clone(),B[0]>=40?j.multiplyScalar(.5):B[0]>=30&&j.multiplyScalar(.8)):j=p,o.colors.push(j,j),B=this.closestFromArray(z,A),B[0]<=45&&B[1]?(j=B[1].faction.planeColor.clone(),B[0]>=40?j.multiplyScalar(.5):B[0]>=30&&j.multiplyScalar(.8)):j=p,o.colors.push(j)}var C=new THREE.Line(o,q,THREE.LinePieces);scene.add(C),c=new Date,console.log("Building the territory reference plane took "+(c.getTime()-d.getTime())+" msec")},buildOldReferencePlane:function(){var a,b,c,d,e,f,g,h,i,j,k,l,m,n,o,p,q,r,s,t,u,v,w=10,x=2*Math.PI/36,y=new THREE.Object3D;r=s=new Date,q=this.furthestPOI(new THREE.Vector3),p=q[0]+50,d=new THREE.LineBasicMaterial({color:6316192,linewidth:1.5,vertexColors:!0,opacity:.6}),f=new THREE.Geometry;for(g=x/2;g<2*Math.PI;g+=x)for(j=Math.cos(g),k=Math.sin(g),t=g-x/2,u=g+x/2,h=Math.cos(t),i=Math.sin(t),a=w/2;p>a;a+=w)b=a-w/2,c=a+w/2,q=this.closestPOI(new THREE.Vector3(a*j,0,-a*k)),q[0]<=35,q[0]<55&&(l=b*h,m=-b*i,n=c*h,o=-c*i,f.vertices.push(new THREE.Vector3(l,0,m)),f.vertices.push(new THREE.Vector3(n,0,o)),f.vertices.push(new THREE.Vector3(l,0,m)),f.vertices.push(new THREE.Vector3(b*Math.cos(u),0,-b*Math.sin(u))));for(v=0;v<f.vertices.length;v++)f.colors[v]=d.color;e=new THREE.Line(f,d,THREE.LinePieces),this.referencePlane=e,scene.add(e),scene.add(y),r=new Date,console.log("Building the territory reference plane took "+(r.getTime()-s.getTime())+" msec")}};var effectFXAA,camera,scene,renderer,composer,map,dpr,shift,ctrl,alt,controls,editor,stats,displayState,localStorage,cameraDefaults={x:0,y:80,z:100,target:new THREE.Vector3(0,10,0),theta:0,phi:THREE.Math.degToRad(55.1),radius:122.2};$(function(){$("#map_ui").tabs({active:0,activate:function(a,b){a.preventDefault();var c=b.newTab.find("a").attr("href");"#editor"===c&&map.canEdit?($("#webgl-container").removeClass().addClass("noselect webgl-container-edit"),window.editor.enabled=!0,window.controls.requireAlt=!0):($("#webgl-container").removeClass().addClass("noselect webgl-container-noedit"),window.editor.enabled=!1,window.controls.requireAlt=!1,"#info"===c&&map.selected()instanceof SCMAP.System&&map.selected().displayInfo()),$("#map_ui").data("jsp").reinitialise()}}),$("#map_ui").jScrollPane({showArrows:!0}),Detector.webgl||Detector.addGetWebGLMessage(),init(),animate()}),Object.values=function(a){var b=[];for(var c in a)a.hasOwnProperty(c)&&b.push(a[c]);return b},jQuery.fn.outerHtml=function(){return jQuery("<div />").append(this.eq(0).clone()).html()};